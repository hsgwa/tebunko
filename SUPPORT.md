...
1. **[README](README.md) の「使い方」と「制約」を見る。** 検索できない文字・変換できないファイルの一覧があります。
2. **ログを見る。** 変換で失敗したファイルと理由は `work\変換エラー.txt` に、変換の経過は `work\変換ログ.txt` に出ます。エラーの意味は [docs/01_変換_8_エラーメッセージ一覧.md](docs/01_変換_8_エラーメッセージ一覧.md) にあります。
3. **設計書を見る。** 画面・変換・検索の細かい動きは [docs/](docs/00_index.md) にあります。
