...
. "${scriptsDir}\tebunko_grep\ui\open_source.ps1"
...
Describe "findSourceFile" -Tag Io {
    ...
    It "記録した場所にあれば、そのパスを返す" {
        ...
        Assert-MockCalled -Scope It showConfirm -Times 0 -Exactly
    }
    ...
    It "記録した場所に無くても、同じ場所を指す別の書き方で見つかれば、その場所を記録して返す" {
        ...
        Assert-MockCalled -Scope It setIndexSourceFolder -Times 1 -Exactly -ParameterFilter { $name -eq "営業" -and $folder -eq "$TestDrive\alias" }
    ...
    It "別の書き方で見つかっても、インデックス名が分からなければ記録しない" {
        ...
        Assert-MockCalled -Scope It setIndexSourceFolder -Times 0 -Exactly
    }
    ...
    It "見つからず、フォルダを選ばなければ `$null（記録した場所の近くを選ぶ画面の初期位置にする）" {
        ...
        Assert-MockCalled -Scope It selectFolder -Times 1 -Exactly -ParameterFilter { $initial -eq "$TestDrive\gone" }
    ...
    It "確認で選ぶのをやめれば `$null" {
        ...
        findSourceFile (newRow) | Should Be $null
        Assert-MockCalled -Scope It selectFolder -Times 0 -Exactly
    ...
    It "記録が無くても、選んだフォルダの中で見つかれば、元のフォルダを記録して返す" {
        ...
        Assert-MockCalled -Scope It setIndexSourceFolder -Times 1 -Exactly -ParameterFilter { $name -eq "営業" -and $folder -eq "$TestDrive\moved\営業" }
        $script:sourceFolderMaps.Count | Should Be 0
    }
    ...
    It "選んだフォルダに無ければ、選んだフォルダを初期位置にしてもう一度聞く" {
        ...
        findSourceFile (newRow) | Should Be $null
        Assert-MockCalled -Scope It showConfirm -Times 2 -Exactly
        Assert-MockCalled -Scope It showConfirm -Times 1 -Exactly -ParameterFilter { $facts[0] -eq "✗ 選んだフォルダの中にありませんでした" }
    }
}
...
Describe "openWithShell" -Tag Io {
    It "通常の開き方は、既定のアプリでそのまま開く" {
        ...
        Assert-MockCalled -Scope It Invoke-Item -Times 1 -Exactly -ParameterFilter { $LiteralPath -eq "$TestDrive\見積.xlsx" }
    }
    ...
    It "読み取り専用で開けなければ、そのまま開いて `$false を返す" {
        ...
        Assert-MockCalled -Scope It Invoke-Item -Times 1 -Exactly
    }
...
Describe "openSource" -Tag Unit {
    ...
    It "元のファイルが見つからなければ開かない" {
        ...
        openSource
        Assert-MockCalled -Scope It openInExcel -Times 0 -Exactly
        Assert-MockCalled -Scope It openWithShell -Times 0 -Exactly
    }
    ...
    It "Excel は該当シート（図形・コメントは元のシート）の該当セルを開く" {
        ...
        Assert-MockCalled -Scope It openInExcel -Times 1 -Exactly -ParameterFilter {
    ...
    It "Excel 以外は既定のアプリで開く（開き方は［開き方］の選択）" {
        ...
        openSource
        Assert-MockCalled -Scope It openWithShell -Times 1 -Exactly -ParameterFilter { $mode -eq ${openModeNew} }
...
Describe "openSourceFolder" -Tag Unit {
    It "行を選んでいない・元のファイルが見つからなければ開かない" {
        ...
        Mock findSourceFile { $null }
        openSourceFolder
        Assert-MockCalled -Scope It Start-Process -Times 0 -Exactly
    }
...
Describe "exportResults" -Tag Io {
    ...
    It "検索する前は出力しない" {
        ...
        Assert-MockCalled -Scope It Invoke-Item -Times 0 -Exactly
    }
    ...
    It "表示中の行を検索結果.txt に書き出して開く" {
        ...
        Assert-MockCalled -Scope It Invoke-Item -Times 1 -Exactly -ParameterFilter { $LiteralPath -eq $resultFile }
...
Describe "画面の操作" -Tag Unit {
    ...
    It "右クリックメニューとボタンは、それぞれの開き方・操作を呼ぶ" {
        ...
        Assert-MockCalled -Scope It openSource -Times 1 -Exactly -ParameterFilter { $mode -eq ${openModeNormal} }
        Assert-MockCalled -Scope It openSource -Times 1 -Exactly -ParameterFilter { $mode -eq ${openModeReadOnly} }
        Assert-MockCalled -Scope It openSource -Times 1 -Exactly -ParameterFilter { $mode -eq ${openModeNew} }
        Assert-MockCalled -Scope It openSource -Times 4 -Exactly
        Assert-MockCalled -Scope It openSourceFolder -Times 2 -Exactly
        Assert-MockCalled -Scope It copySelectedRows -Times 1 -Exactly
        Assert-MockCalled -Scope It copySourcePath -Times 1 -Exactly
        Assert-MockCalled -Scope It exportResults -Times 1 -Exactly
    }
    ...
    It "［開き方］を選ぶと設定に保存する（起動時の読み込みでは保存しない）" {
        ...
        Assert-MockCalled -Scope It writeOpenMode -Times 0 -Exactly

        $script:loadingOpenMode = $false
        & $ui.OpenModeCombo.Handlers["SelectionChanged"]
        Assert-MockCalled -Scope It writeOpenMode -Times 1 -Exactly -ParameterFilter { $mode -eq ${openModeReadOnly} }
        $ui.MenuOpenReadOnly.InputGestureText | Should Be "Enter"
    }
    ...
    It "Enter は、見出しの行では閉じる・開く、ほかの行では元のファイルを開く" {
        ...
        Assert-MockCalled -Scope It toggleFileGroup -Times 1 -Exactly
        Assert-MockCalled -Scope It openSource -Times 0 -Exactly

        $ui.ResultGrid.SelectedItem = newRow
        & $ui.ResultGrid.Handlers["PreviewKeyDown"] $null ([pscustomobject]@{ Key = "Return"; Handled = $false })
        Assert-MockCalled -Scope It openSource -Times 1 -Exactly
    }
    ...
    # WPF の要素は STA のスレッドでしか作れない（powershell.exe 5.1 は既定で STA）
