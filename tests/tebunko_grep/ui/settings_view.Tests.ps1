...
Describe "newWorkspaceConfirm" -Tag Unit {
    It "空のフォルダなら、取り込み直すことと、今のワークスペースは残ることを伝え、変えるボタンを 1 つ出す" {
    ...
    It "空でなければ警告し、中身の数と例を出し、中に workspace を作るか・そのまま使うかを選ばせる" {
        ...
        @($confirm.Facts | ForEach-Object { $_.Kind }) -join "," | Should Be "warn,kept"
        ...
        $confirm.Hint | Should Match "^空のフォルダを選び直すときは［キャンセル］"
    }
    ...
    It "インデックスがあれば前のワークスペースとして使えると伝え、中に workspace を作れなければ、その選択肢を出さない" {
        $confirm = newWorkspaceConfirm "D:\データ" "C:\tool\work" 3 @("index", "取り込み一覧.tsv", "インデックス作成ログ.txt") $false $true $false
        @($confirm.Facts | ForEach-Object { $_.Kind }) -join "," | Should Be "warn,kept,kept"
