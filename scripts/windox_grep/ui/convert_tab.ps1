...
function findRunningConversion {
    # このツールの変換（windox_grep\convert.ps1）が実行中なら、そのプロセスを返す（画面を閉じて開き直した場合など）
    $script = "${PSScriptRoot}\windox_grep\convert.ps1"
...
function startConversion {
    ...
    $arguments = "-NoProfile -ExecutionPolicy RemoteSigned -WindowStyle Hidden -File `"${PSScriptRoot}\windox_grep\convert.ps1`" -ConfirmTargets"
