# windox_grep の画面で使う型（検索結果・プレビュー・インデックス一覧など）。
...
# 検索対象の1件（windox_grep\search\search_run.ps1 の getIndexTsvFiles に渡す）
class SearchTarget {
    [string]$Root
    [string]$RelPath
    [bool]$Recurse
...
# 検索対象から外したフォルダ（windox_grep\core\settings_grep.ps1 の readSearchExcludes / writeSearchExcludes と同じ項目）
class SearchExclude {
    [string]$Path
    [bool]$Subfolders
}
