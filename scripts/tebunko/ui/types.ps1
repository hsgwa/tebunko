# tebunko の画面で使う型（検索結果・プレビュー・インデックス一覧など）。
...
# 検索対象の1件（tebunko\search\pack_search.ps1 の getIndexPackFiles に渡す）
class SearchTarget {
    [string]$Root
    [string]$RelPath
    [bool]$Recurse
...
# 検索対象から外したフォルダ（tebunko\core\settings.ps1 の readSearchExcludes / writeSearchExcludes と同じ項目）
class SearchExclude {
    [string]$Path
    [bool]$Subfolders
}
