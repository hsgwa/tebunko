...
- 該当する記録（`work\インデックス作成ログ.txt`、画面に出たエラーのメッセージ。機密情報が含まれる場合は該当部分を伏せてください）
...
- 内容を確認して再現を試みます。
- 修正する場合は、原因と影響範囲を [docs/safety/index.md](../docs/safety/index.md) と該当の設計書に反映し、再発を防ぐテストを `tests/` に追加します。
- 修正の配布は、`tools\new_release_files.ps1` で作るカタログとハッシュ一覧を添え、zip の来歴を Sigstore で署名して行います。受け取った側が配布物の完全性を確認できます（[README の「安全性」](../README.md#安全性)）。

## 利用者側でお願いしたいこと

本ツールは、**インデックス（`work\index\` の TSV）に元の文書の本文を平文で保持**します。元のファイルのアクセス権は引き継がれません。次の点に注意してください（詳細は [docs/safety/disclosure.md](../docs/safety/disclosure.md) の「インデックスが元文書の本文を保持する（情報の集約）」）。
