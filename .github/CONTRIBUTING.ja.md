...
- **`scripts/shared/` は個々のツールを知らない**ようにします。ツール同士も互いを読み込みません（両方を読み込んでよいのは、画面を組み立てるだけの `tebunko/` だけです）。
- **判断層**（`*_view.ps1`・`index_name.ps1`・`search_query.ps1`・`text.ps1`・`tebunko_diff/diff/*.ps1`）は画面（WPF）に触れません。画面に出す文言や可否の判定はここに置き、テストを書きます。
