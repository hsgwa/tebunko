...
- **`scripts/shared/` does not know about individual tools.** Tools do not load each other either (only `tebunko/`, which assembles the window, may load both).
- **The decision layer** (`*_view.ps1`, `index_name.ps1`, `search_query.ps1`, `text.ps1`, `tebunko_diff/diff/*.ps1`) does not touch the screen (WPF). Put the text shown on screen and the decisions about what is allowed here, and write tests for them.
- Load every file you add from an entry point (`shared/shared.ps1`, `tebunko_grep/lib.ps1`, `tebunko_diff/lib.ps1`, `tebunko/gui.ps1`, `indexer.ps1`, `differ.ps1`).
