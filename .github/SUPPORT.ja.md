...
2. **ログを見る。** インデックス作成の経過と、取り込みに失敗したファイルの理由は `work\インデックス作成ログ.txt` に出ます。インデックス作成を続けられなかったときのエラーは、画面に出るほか、同じログにも出ます。エラーの意味は [docs/design/indexer/errors.md](../docs/design/indexer/errors.md) にあります。
