...
1. **[README](../README.md) の「使い方」と「制約」を見る。** 検索できない文字・取り込めないファイルの一覧があります。
   - **画面（ウィンドウ）が開かず、黒い画面で「制限モードで起動します」と出るとき**は、その PC のアプリケーション制御（AppLocker・WDAC）が原因です。制限モードのままでも検索できます（[README の制限モード](../README.md#制限モード)）。いつもの画面を使うには、管理者に [管理者向けの案内](../docs/05_制限モード_1_管理者向け.md) を見てもらってください。
