...
#
# 出力:
#   win_grep.cat       カタログ（各ファイルの SHA256。Test-FileCatalog で検証する）
...
#   Set-AuthenticodeSignature -FilePath .\win_grep.cat -Certificate $cert
param (
    [string]$OutDir
)
...
$targets = @(
    ...
    (Join-Path $rootDir "win_grep.bat")
)

$catalogPath = Join-Path $OutDir "win_grep.cat"
...
$lines.Add("# win_grep 配布物の SHA256（$(Get-Date -Format 'yyyy/MM/dd HH:mm:ss') 時点）")
