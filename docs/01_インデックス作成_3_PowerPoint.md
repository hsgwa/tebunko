...
| `スライドNNN（非表示）` | 非表示スライド（`<p:sld show="0">`） |
...
- スライドマスター・レイアウトのテキスト、SmartArt・グラフ内の文字、埋め込みオブジェクトは読まない。
...
| 2 | スライドマスター・レイアウトのテキスト、SmartArt・グラフ内の文字、埋め込みオブジェクト（Excel の表など）は検索対象にならない | ○ |
