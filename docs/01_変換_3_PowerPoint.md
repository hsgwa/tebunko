...
| # | 内容 | 確度 |
|---|---|---|
| 1 | 旧形式（`.ppt`）・パスワード付きの PowerPoint ファイルの変換には PowerPoint が必要。インストールされていない場合は変換一覧に「失敗」と記録される | ○ |
| 2 | スライドマスター・レイアウトのテキスト、SmartArt・グラフ内の文字、埋め込みオブジェクト（Excel の表など）は検索対象にならない | ○ |
| 3 | PowerPoint を利用者が使用中の場合、旧形式の変換にはそのアプリが使われる（終了はさせない。01_変換_6_共通処理とアプリ管理.md の 5.1 節） | △ |
