...
| `scripts/shared/ui/folder_dialog.ps1` | Windows 標準のフォルダ選択（`selectFolder`。3.2.1）。WinForms 内部の `IFileDialog` をリフレクションで呼んで開き、開けなければ `OpenFileDialog` でフォルダの中に入って選んでもらう（[実行時コンパイル（csc.exe）を使わない](#実行時コンパイルcscexeを使わない)）。フォルダのドラッグ＆ドロップの判定（`getDroppedFolders`・`onFolderDragOver`）も置く |
...
| `scripts/tebunko/ui/settings_tab.ps1` | ［8 設定］タブ（[［8 設定］タブ](../architecture/settings-file.md#8-設定タブ)）。ワークスペースを変えたら、その場で切り替える |
...
| `getSourceLocation` / `resolveSourcePath` | 結果 1 件, フォルダ → 対応 のキャッシュ | `@{Name; Folder; Rest; Known}` / string / `$null` | 元のファイルのインデックス名・場所・パス（[元のフォルダの特定（インデックスを別の PC・場所で使う場合）](search-tab.md#元のフォルダの特定インデックスを別の-pc場所で使う場合)） | 画面 |
| `findMovedSource` / `setIndexSourceFolder` | 選んだフォルダ, 相対フォルダ, ファイル名 / インデックス名, フォルダ | `@{Path; Root}` / `$null` | 元のファイルが見つからないとき、選んだフォルダから探し、インデックス名に対する元のフォルダを設定に記録する（[元のファイルが見つからないとき（元のフォルダを設定する）](search-tab.md#元のファイルが見つからないとき元のフォルダを設定する)） | 画面 |
| `getTargetFolders` / `writeTargetFolders` / `normalizeFolderPath` | – / `@{ Name; Path; Enabled }` の配列 / パス | 同上 / – / パス | インデックス一覧（`setting.config` の `targetFolders`。インデックス名と置き場所を分けて持つ）。［編集…］は Name・Path を書き換える（[［1 インデックス管理］タブ](index-tab.md)） | 画面・インデックス作成 |
| `readSearchOption` / `writeSearchOption` | – / 変える項目の hashtable | `@{ UseRegex; CaseSensitive; FileFilter; IncludeShapes; IncludeComments }` | `setting.config` の `useRegex` / `caseSensitive` / `fileFilter` / `includeShapes` / `includeComments`（[画面での読み書き](../architecture/settings-file.md#画面での読み書き)） | 画面 |
| `readOpenMode` / `writeOpenMode` | – / 開き方 | 開き方（`normal` / `readOnly` / `new`。知らない値は `normal`） | `setting.config` の `openMode`（[画面での読み書き](../architecture/settings-file.md#画面での読み書き)、[元のファイルを開く](search-tab.md#元のファイルを開く)） | 画面 |
| `getSearchIndexes` | – | `@{ Name; Path; SourcePath }` の配列 | インデックスの一覧（`work\index` 直下のフォルダ）。検索対象のツリーの一番上の項目に使う（[検索対象のツリー（No.13）](search-tab.md#検索対象のツリーno13)） | 画面 |
| `newSearchService` / `newSearchRequest` | lib.ps1 のパス, 読んだ内容の使い回し, 照合のスレッド数 / ワード, 文字どおりか, 検索対象, 上限, 検索条件, 高速検索を使うか | `SearchService` / 要求 | 検索の司令のスレッド（画面を開いている間 1 つ）と、検索 1 回分の要求（`Request` で渡す。前の要求は取り消す）（12.1。[寿命](../architecture/threads.md#寿命)） | 画面 |
| `newIndexingSession` / `newIndexerChannel` | indexer.ps1 のパス, 受け渡しの口 / 失敗分も再取り込みするか, 確認するか, 読み取りのスレッドの数 | `IndexingSession` / 受け渡しの口 | インデックス作成 1 回分のスレッドと受け渡しの口（12.1。[画面とインデクサの受け渡し](../architecture/threads.md#画面とインデクサの受け渡し)） | 画面 |
| `readIndexingProgress` / `requestIndexingStop` / `answerIndexingPlan` | 受け渡しの口 / 受け渡しの口 / 受け渡しの口, 返事 | 進み具合 / – / – | 進み具合を読む（[インデックス作成の進み具合](index-tab.md#インデックス作成の進み具合)）、中止を求める（[中止・終了・ログ](index-tab.md#中止終了ログ)）、確認の返事を返す（[インデックス作成の確認ダイアログ](index-tab.md#インデックス作成の確認ダイアログ)） | 画面 |
| `testIndexerRunning` | – | bool | 画面を使わずに起動したものを含め、この `work` でインデックス作成が動いているか（ワークスペースを変える前に調べる） | 画面（［8 設定］） |
...
- 多重起動は、ツールの配置フォルダごとの名前付き Mutex で防ぐ。2 つ目のプロセスは、同じくフォルダごとの名前付きイベント（`Local\tebunko_gui_activate_<フォルダのハッシュ>`）を合図して終了し、すでに開いている画面が `DispatcherTimer`（0.3 秒間隔）でそれを確認して `Window.Activate()`（＋最小化なら復帰）で前面に出す。別アプリが前面のときはフォーカスを奪えないことがある（OS の制限。P/Invoke の `SetForegroundWindow` は使わない。12.2）。
...
| 第 2 段階 | 画面だけで操作する。インデックス作成は画面のプロセスの中のスレッドで実行し、［中止］・終了後の表示・［ログを開く］・インデックス作成中に閉じるときの確認（止めてから閉じる。3.7）。［インデックス作成を開始］で取り込み対象を数えて、インデックスごとの件数を確認する（3.4。失敗分の再取り込みもここで選ぶ）。画面とインデクサの受け渡しは、メモリ上の受け渡しの口（`newIndexerChannel`。確認の返事・中止・進み具合・エラー・終了コード 0 / 1 / 2）で行い、ファイルには `インデックス作成ログ.txt` だけを書く（[画面とインデクサの受け渡し](../architecture/threads.md#画面とインデクサの受け渡し)） | **実装済み** |
...
| # | 事項 | 選択肢 | 決定 |
|---|---|---|---|
| U1 | 入口の名前 | `0_設定.bat` / `tebunko.bat` / `tebunko.bat` など | **`tebunko.bat`**。画面がすべての操作の入口で `.bat` もこれ 1 本だけのため、番号を付けない。ツールが増えても入口は 1 つにするため、画面の表題（`tebunko`）ではなくツール全体の名前（`tebunko`）にする |
...
> **16. テスト**（16.1 単体テスト・16.2 画面の確認）→ [テスト](../testing/index.md)
