...
| 項目 | 内容 |
|---|---|
| 状態 | **第 1・2 段階を実装済み**（[既存スクリプトへの影響と段階](implementation.md#既存スクリプトへの影響と段階)）。第 3 段階は未実装 |
| 起動用バッチ | `tebunko.bat`（名称は [未決事項](implementation.md#未決事項) U1）。PowerShell の窓を残さずに画面だけを開く（[配布と実行ポリシー（Mark-of-the-Web）](common.md#配布と実行ポリシーmark-of-the-web)） |
