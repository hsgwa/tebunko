...
- **5.2.2** インデックスの TSV の名前・作成・配置と、検索（`shared/core/fs.ps1`・`text.ps1`、`tebunko_grep/index/`・`search/`）
...
| `encodeIndexPlace` | place | string | TSV のファイル名に入れる場所を符号化する（ファイル名禁止文字・制御文字・`_`・`%` を `%XX` に）。`encodeIndexPlace` / `decodeIndexPlace` / `toIndexFileName` は `shared/office/place_name.ps1`（抽出の `writeUnits` が使うため shared に置く。比較も同じ名前の TSV を読む） | [01_インデックス作成_7 6.2 節](01_インデックス作成_7_出力TSVと既知の問題.md#62-場所の符号化encodeindexplace--decodeindexplace) | toIndexFileName |
| `decodeIndexPlace` | place | string | `encodeIndexPlace` の `%XX` を元に戻す（それ以外の `%` はそのまま） | 同上 | splitIndexFileName |
...
| `splitIndexTsvPath` | relPath | `@{Book; Place; RelDir}` | インデックスフォルダからの TSV の相対パスを、元のファイル名・場所・元のファイルのあるフォルダに分ける。今の形式（`<ファイル名.xlsx>\<場所>.tsv`）と以前の形式（フラット）の両方を扱う | [02_検索.md 5.3 節](02_検索.md#53-ファイル名場所の分解splitindextsvpath) | 画面・検索処理（`newTsvFiles` は同じ規則をその場で展開する） |
...
| `splitObjectPlace` | place | `@{Base; Kind}` | 図形・コメントの場所（`<元の場所>[図形]` 等）を、元の場所と種類に分ける。ふつうの場所は Kind が空 | [01_インデックス作成_7](01_インデックス作成_7_出力TSVと既知の問題.md#61-配置命名規則) 6.1「図形・コメントの場所」 | 元のファイルを開く |
...
| `readTsvContext` | path, lineNumber, before（既定 3）, after（既定 3） | `@{LineNumber; Line}` の配列 | TSV の lineNumber 行目と前後の行を返す（行の数え方は `searchIndex` と同じ）。`StreamReader` で先頭から目的の行まで読む（TSV は元のファイル 1 つ分で、通常は数千行までのため十分速い）。読めなければ空 | [03_画面_2_検索タブ.md](03_画面_2_検索タブ.md) 4.7 | 画面 |

**インデックスへの配置（`tebunko_grep/index/index_store.ps1`）**

| 関数 | 入力 | 出力 | 概要 | 詳細 | 使用元 |
|---|---|---|---|---|---|
| `getIndexTsvCounts` | dir（既定 `$indexDir`） | 相対パス → TSV 数の辞書 / `$null` | インデックスの中のフォルダごとの TSV の数を数える（フォルダ・TSV をそれぞれ 1 回ずつ列挙する。TSV の無いフォルダは 0 件）。キーは取り込み一覧の相対パスと同じ。列挙できなければ `$null` | [01_インデックス作成_5](01_インデックス作成_5_クロール.md#取り込み対象の決定createtargetlist) | インデックス作成（取り込み対象の決定） |
...
**検索（`tebunko_grep/search/search_query.ps1`・`search_run.ps1`）**

```mermaid
flowchart LR
    A["getIndexTsvFiles<br>TSV を列挙"] --> B["searchIndex"]
    B --> C["newSearchRegex / newFileFilter / newPlaceExclude<br>条件を正規表現に"]
    B --> D["newTsvFiles<br>対象ファイル・図形とコメントで絞る"]
    D --> E["searchTsvFiles<br>読み込みと照合（件数が多ければ並列）"]
    E --> F["Hits"]
...
| `newSearchRegex` | word, simpleMatch, caseSensitive | `@{Regex; SimpleMatch; TextRegex; ScanMode}` | 検索条件から照合用の正規表現を作る（文字どおりならエスケープ、大文字と小文字を区別しないなら IgnoreCase、正規表現として不正なら文字どおりにする）。1 行の照合は 5 秒で時間切れ。TextRegex は TSV の全文にかける正規表現（Multiline）、ScanMode はそれを全文にかけてよいか（`getRegexScanMode`） | [02_検索.md 4.2](02_検索.md#42-検索条件サクラエディタの-grep-にならう) | 検索・画面（一致箇所の強調） |
| `getRegexScanMode` | pattern | string（`lines` / `filter` / `scan`） | 正規表現を TSV の全文にかけて、1 行ずつの照合と同じ結果になるかを判定する。`lines` は全文での一致の位置から行が分かる、`filter` は全文で一致しない TSV を読み飛ばせる、`scan` は 1 行ずつ照合する。分からない書き方は安全側（`filter` か `scan`）に倒す | [02_検索.md 4.3](02_検索.md#43-検索の実装速度) | newSearchRegex |
| `newFileFilter` | filter | `@{Include; Exclude}` | 対象ファイルの指定（`*.xlsx;見積;!*old*`）を、元のファイル名に対する正規表現にする（無い側は `$null`） | [02_検索.md 4.2](02_検索.md#42-検索条件サクラエディタの-grep-にならう) | `searchIndex` |
...
| `getIndexTsvFiles` | folders（既定 `$indexDir`。フォルダの文字列、または `@{Root; RelPath; Recurse}`（画面のツリーで選んだ範囲。`Recurse` が `$false` なら直下だけ）の配列）, onProgress（2000 件ごとに `{ param($count) }` を呼ぶ） | `@{Folders; Files}` | 検索対象の TSV を列挙する。結果の相対パスは `Root` から求める。Folders はフォルダごとの `@{Path; Root; Exists; Count}`、Files は TSV のフルパス → `@{Root; RelPath; LongPath; Ticks; Size}`（パス順・重複なし。LongPath は `\\?\` 付きのパス、Ticks・Size は読んだ内容を使い回してよいかの判定に使う）。検索を始めるまでの待ち時間になるため、`Get-ChildItem` ではなく .NET の列挙を使い、相対パスも切り出しで求める | [02_検索.md 4.3](02_検索.md#43-検索の実装速度) | 検索・画面 |
| `findTsvFilesParallel` | longDir | FileInfo の一覧 | フォルダ以下の `*.tsv` を、直下のフォルダごとに別スレッドで並行して列挙する（直下のフォルダが少なければ 1 スレッド） | 同上 | getIndexTsvFiles, getIndexSummary |
...
| `newTsvFiles` | tsvFiles（getIndexTsvFiles の Files）, include, exclude, excludePlace | 検索対象の一覧 | TSV ごとに元のファイル名（Book）・場所（Location）・相対フォルダ（RelDir）を付け、対象ファイルの指定と図形・コメントの指定で絞る。TSV の数だけ呼ぶと遅いため、`splitIndexTsvPath` と同じ規則をその場で展開する | 同上 | searchIndex |
| `searchTsvFiles` | files, start, count, regex, max, textRegex, scanMode, cache | ヒットの一覧 | files の start から count 件を読み、regex に一致する行を返す（1 行に複数一致しても 1 件）。`ScanMode` に応じて全文に 1 回照合し、1 行ずつの照合を減らす。インデックス作成中の TSV も読めるよう共有して開き、読めない TSV は飛ばす | 同上 | searchIndex（並列検索のスレッドでも動く） |
| `searchIndex` | word, tsvFiles, simpleMatch, limit, chunkSize, onProgress（`{ param($done, $total, $newHits) }`）, shouldStop, caseSensitive, fileFilter, workerCount（並列のスレッド数。0 は自動）, cache（`newTsvTextCache`）, includeShapes, includeComments（図形・コメントの場所も検索するか。`newPlaceExclude`） | `@{Hits; SimpleMatch; Total; Truncated; Cancelled}` | TSV を検索する（照合は `searchTsvFiles`＝.NET の `StreamReader`＋`[regex]`）。Hits は PSCustomObject（Root・RelPath・RelDir・FileName・Book・Location・LineNumber・Line）。Total は対象ファイル・図形とコメントの指定で絞った後の TSV の数。chunkSize 件ずつ検索して進捗を知らせ、上限・中止に対応する。TSV が多ければ複数スレッド（最大 8）で並行して検索する（結果の順は変わらない）。照合が時間切れになれば例外にする | [02_検索.md](02_検索.md#4-検索処理) | 検索・画面 |
| `newTsvTextCache` | maxChars（既定 6,400 万文字） | `@{Texts; Chars; MaxChars}` | 検索で読んだ TSV の内容を次の検索で使い回す入れ物。更新日時・サイズが変わった TSV は読み直す | [02_検索.md 4.3](02_検索.md#43-検索の実装速度) | 画面 |
