...
| `%LOCALAPPDATA%\tebunko\<鍵>` 配下 | ツールを置いたフォルダに書き込めないとき（Program Files・読み取り専用の共有フォルダ）だけ、`setting.config` と既定の `work/` をここに置く。鍵はツールのフォルダのパスから作る 16 文字（[データの置き場所](../design/architecture/layout.md#データの置き場所settingconfigwork)） | `shared/core/data_dir.ps1:31` |
