...
| `$workspace.Dir` | `setting.config` の `workspaceFolder`（4.1）。空なら既定の `%USERPROFILE%\Documents\tebunko`（`getDefaultWorkDir`。OneDrive にリダイレクトされた「ドキュメント」ではなく、プロファイルの直下の Documents） | `scripts/tebunko/core/paths.ps1`（`getWorkDir`） |
