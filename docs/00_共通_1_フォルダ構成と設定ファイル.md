...
| `scripts/tebunko_grep/restricted/` | スクリプト | 制限モード（制限言語モードの PC で使うコンソールの版）の部品。起動の確認の判断（`startup_view.ps1`）と事実集め（`startup_check.ps1`）、制限モードの読み込み口（`lib_restricted.ps1`） |
