...
| パス | 種別 | 説明 |
|---|---|---|
| `tebunko_grep.bat` | 起動用バッチ | 画面を開く（インデックス作成・検索・プロセス停止）。利用者が起動するのはこれだけ。`scripts` の中のファイルから Mark-of-the-Web を外し（`Unblock-File`。[04_安全性.md](04_安全性.md) の 4.2）、`start.ps1` でいつもの画面を開けるかを確かめてから、`conhost.exe` 経由で PowerShell を `-ExecutionPolicy RemoteSigned -WindowStyle Hidden` で起動して `gui.ps1` を開く。既定のターミナルが Windows Terminal だと `-WindowStyle Hidden` が効かず、PowerShell の窓が残るため `conhost.exe` を通す。画面を開けないときは、バッチのコンソールで制限モードを動かす（[03_画面_5_共通仕様.md 10.1](03_画面_5_共通仕様.md#101-配布と実行ポリシーmark-of-the-web)） |
...
ソースは**文脈**（`shared/` = どのツールからも使う、`tebunko_grep/` = このツール固有）と**層**で分ける。フォルダごとの中身と読み込み口は [5 章](00_共通_2_共通モジュール.md#5-共通モジュール)。
...
| `scripts/tebunko_grep/` | スクリプト | tebunko_grep 固有の処理と画面（`core/`・`index/`・`indexer/`・`search/`・`ui/`・`xaml/`） |
| `scripts/tebunko_grep/start.ps1` | スクリプト | 起動の確認の起動口。`tebunko_grep.bat` が最初に動かし、いつもの画面を開けるか（言語モード・WPF）を確かめる。開けないときは理由を出して制限モードで起動する。制限言語モードでも動く書き方で書く |
| `scripts/tebunko_grep/start_failed.txt` | 文言 | `start.ps1` 自体を動かせなかったとき（グループポリシーの実行ポリシーなど）に、`tebunko_grep.bat` が表示する案内。バッチは ASCII で書くため、日本語の文言はこのファイルに置く |
| `scripts/tebunko_grep/restricted/` | スクリプト | 制限モード（制限言語モードの PC で使うコンソールの版）の部品。起動の確認の判断（`startup_view.ps1`）と事実集め（`startup_check.ps1`）、制限モードの読み込み口（`lib_restricted.ps1`）、検索（`restricted_search.ps1`）、検索結果のブック（`result_book.ps1`・`result_book_view.ps1`）、コンソール（`search_console.ps1`・`console_view.ps1`）。[05_制限モード.md](05_制限モード.md) |
...
| `scripts/tebunko_grep/lib.ps1` | スクリプト | 画面以外の部品の読み込み口 |
| `scripts/tebunko_grep/xaml/` | 画面定義 | tebunko_grep の画面定義（`tebunko_grep.xaml`・タブ・ダイアログ） |
...
設定は、ツール直下の `setting.config`（内容は JSON）の 1 ファイルにまとめ、画面が読み書きする。利用者が手で編集する前提にはしない。インデクサ（別プロセス）は、画面が保存したクロール対象フォルダをこのファイルから読む。読み書きの関数は `scripts/tebunko_grep/core/settings_grep.ps1` にある。
...
| `startupMode` | `auto` / `restricted` / `clm` | `auto` | 起動のしかた。`auto` はいつもの画面を開けるかを調べて決める。`restricted` は画面を開ける PC でも制限モードのコンソールで起動し、`clm` は模擬の制限言語モードにしてから制限モードで起動する（どちらも**手元で制限モードを確かめるための設定**。[05_制限モード.md 8 章](05_制限モード.md#8-手元で制限モードを確かめる)）。知らない値は `auto` とする。画面では変えられない（設定ファイルに直接書く） | `readStartupModeSetting`（`start.ps1` が起動の判断だけに読む） |
