...
| パス | 種別 | 説明 |
|---|---|---|
| `tebunko.bat` | 起動用バッチ | 画面を開く（インデックス作成・検索・比較・プロセス停止）。利用者が起動するのはこれだけ。`scripts` の中のファイルから Mark-of-the-Web を外し（`Unblock-File`。[04_安全性.md](04_安全性.md) の 4.2）、`conhost.exe` 経由で PowerShell を `-ExecutionPolicy RemoteSigned -WindowStyle Hidden` で起動して `scripts/tebunko/gui.ps1` を開く。既定のターミナルが Windows Terminal だと `-WindowStyle Hidden` が効かず、PowerShell の窓が残るため `conhost.exe` を通す |
| `setting.config` | 設定 | 画面が保存する設定（JSON。無ければ既定値で動き、画面で設定を保存したときに作成する。git 管理外。4 章） |
| `work/` | 自動生成 | インデックス・状態ファイル・ログ（3.2）。git 管理外。削除すると全件取り込み直しになる |

#### スクリプト（`scripts/`）

ソースは**文脈**（`shared/` = どのツールからも使う、`tebunko_grep/` = 検索、`tebunko_diff/` = 比較、`tebunko/` = 両方を 1 つの画面に組み立てるだけ）と**層**で分ける。フォルダごとの中身と読み込み口は [5 章](00_共通_2_共通モジュール.md#5-共通モジュール)。
...
| `scripts/tebunko/` | スクリプト | 検索と比較を 1 つの画面に組み立てるだけの場所 |
...
| `scripts/tebunko_grep/` | スクリプト | 検索（tebunko_grep）固有の処理と画面の中身（`core/`・`index/`・`indexer/`・`search/`・`ui/`・`xaml/`） |
| `scripts/tebunko_grep/indexer.ps1` | スクリプト | インデックス作成の起動口（[01_インデックス作成.md](01_インデックス作成.md)）。画面がウィンドウ無しで起動する |
| `scripts/tebunko_grep/lib.ps1` | スクリプト | 検索の画面以外の部品の読み込み口 |
| `scripts/tebunko_grep/xaml/` | 画面定義 | 検索のタブの中身とダイアログ（`tab_index.xaml`・`tab_search.xaml` ほか） |
| `scripts/tebunko_diff/` | スクリプト | 比較（tebunko_diff）固有の処理と画面の中身（`core/`・`diff/`・`job/`・`ui/`・`xaml/`。[05_比較.md](05_比較.md)） |
| `scripts/tebunko_diff/lib.ps1` | スクリプト | 比較の画面以外の部品の読み込み口 |
| `scripts/tebunko_diff/differ.ps1` | スクリプト | 比較の抽出プロセス。画面がウィンドウ無しで起動する |

#### ドキュメント・配布物に同梱するもの

| パス | 種別 | 説明 |
|---|---|---|
| `README.md`・`LICENSE` | ドキュメント | 使い方の入口とライセンス。配布 zip に同梱する |
| `.github/SECURITY.md`・`.github/SECURITY.ja.md` | ドキュメント | 安全性の説明の入口と、脆弱性の連絡先・対応方針（英語版が正、`.ja.md` が日本語版）。配布 zip では両方を直下に置く |
...
| `tools/new_release_package.ps1` | 配布用 | 配布する zip（`tebunko-<バージョン>.zip`。展開すると `tebunko/` フォルダになる。本体（`scripts/`・`tebunko.bat`）・LICENSE・README・SECURITY（英語・日本語）・SBOM・カタログ・ハッシュ一覧）を `work/release/` に作る。`v` で始まるタグを push すると `.github/workflows/release.yml` が実行し、GitHub Release に載せる（[6 章](00_共通_3_テスト.md#ci)） |
...
| パス | 説明 |
|---|---|
| `work/index/` | 取り込み済み TSV。クロール対象フォルダごとに `work/index/<インデックス名>/` に分かれ、その下は元のファイル 1 つにつき 1 フォルダ（`<ファイル名.xlsx>/<場所>.tsv`） |
...
| `work/tebunko_diff/比較結果.txt` | ［2 比較］の［結果をファイルに出力］で書き出す比較結果（[05_比較.md](05_比較.md)） |
...
できた TSV をインデックスに入れるときは、`work/取り込み出力/<PID>` にいったん集めてからフォルダごと入れ替える（`publishIndexFiles`）。途中で強制終了しても作りかけのインデックスが残らない。フォルダごと移すには同じドライブである必要があるため、作業領域（`%TEMP%`）とは別に `work` の中に置く。どちらのフォルダも、終了時と次回の開始時（終了済みのプロセスの分）に削除する。
...
|---|---|---|---|---|---|
| `setting.config` | 設定 | 画面 | 画面・インデックス作成 | UTF-8（BOM なし）、JSON | クロール対象フォルダ・検索対象のツリーでチェックを外したフォルダ・元のフォルダ・検索条件・開き方・比較の設定（4 章）。PC ごとの設定 |
...
| `work/index/` | インデックス | インデックス作成 | 検索 | UTF-8（BOM 付き）、CRLF | シート・ページ・スライドごとの TSV と、図形・コメントの TSV（`<元の場所>[図形]`・`<元の場所>[コメント]`） |
...
各設定の意味は、使用する処理の設計書に記載する（クロール対象フォルダ → [01_インデックス作成.md](01_インデックス作成.md)、検索対象インデックス → [02_検索.md](02_検索.md)、比較 → [05_比較.md](05_比較.md)、画面での扱い → [03_画面_5_共通仕様.md 8 章](03_画面_5_共通仕様.md#8-設定ファイルの読み書き)）。

---

## 4. 設定ファイル `setting.config`（`readSettings`）

設定は、ツール直下の `setting.config`（内容は JSON）の 1 ファイルにまとめ、画面が読み書きする。検索と比較の設定も同じファイルに入れる。利用者が手で編集する前提にはしない。インデクサ（別プロセス）は、画面が保存したクロール対象フォルダをこのファイルから読む。ファイルの読み書き（`readSettingsData`・`mergeSettingValues`・`writeSettingsFile`）は `scripts/shared/core/settings.ps1`、キーと既定値はツールごと（検索は `scripts/tebunko_grep/core/settings_grep.ps1` の `newSettings`、比較は `scripts/tebunko_diff/core/settings_diff.ps1` の `newDiffSettings`）に持つ。
