...
| `installer/tebunko.iss` | 配布用 | インストーラー（`tebunko-setup-<バージョン>.exe`）を作る Inno Setup 6 のスクリプト。管理者権限なしで `%LOCALAPPDATA%\Programs\tebunko` に入れ、スタートメニューとアンインストールに登録する（[04_安全性.md](04_安全性.md) の 4.6）。BOM 付き UTF-8・CRLF |
...
| `tools/new_installer.ps1` | 配布用 | インストーラー（`tebunko-setup-<バージョン>.exe`）を `work/release/` に作る。起動口 `tebunko.exe` を Windows 標準の `csc.exe`（.NET Framework）でビルドし、`scripts/`・`LICENSE` と並べて Inno Setup 6 の `ISCC.exe` に渡す。`release.yml` が実行する。手元で作るときは Inno Setup 6 を入れておく（`-Iscc` で場所を指定できる） |
