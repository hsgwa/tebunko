...
| `scripts/windox_grep/core/` | windox_grep のパス定義（`paths_grep.ps1`）・設定ファイル（`settings_grep.ps1`） |
| `scripts/windox_grep/index/` | インデックス名の決め方（`index_name.ps1`）・インデックスの作成と集計（`index_store.ps1`） |
| `scripts/windox_grep/convert/` | 変換対象の決定（`convert_plan.ps1`）・変換の状態ファイル（`convert_state.ps1`）・1 ファイルの変換（`convert_office.ps1`）・後始末（`index_migrate.ps1`） |
| `scripts/windox_grep/search/` | 検索条件（`search_query.ps1`）・検索の実行（`search_run.ps1`）・元のファイルの場所（`source_map.ps1`） |
| `scripts/windox_grep/ui/` | タブごとの画面（`*_tab.ps1` ほか）と、その判断層（`*_view.ps1`） |
...
| 状態層（ファイル・COM を読み書きする） | `convert_state.ps1`・`index_store.ps1`・`search_run.ps1` | できる（タグ `Io`。`$TestDrive` を使う） |
| 画面層（`$ui` を触る） | `gui.ps1`・`*_tab.ps1`・`shell.ps1` | しない（手で確かめる） |

依存の向きは一方向にする。**`shared/` はツール（`windox_grep/`）を知らない。** ツール同士も互いを読み込まない。この決まりは `tests/meta/layers.Tests.ps1` で機械的に確かめる。

読み込み口は次の 2 つ。ファイルを足したら、どちらかから読み込む（読み込み漏れもメタテストで検出する）。

| 読み込み口 | 読み込むもの | 使う側 |
|---|---|---|
| `scripts/shared/shared.ps1` | 共通基盤（`core/`・`office/` のうち COM を使わないもの） | `windox_grep/lib.ps1` |
| `scripts/windox_grep/lib.ps1` | 上記＋ windox_grep の `core/`・`index/`・`convert/`・`search/` | 画面・変換処理・テスト・画面が起こす別スレッド |

画面の部品（`shared/ui/`・`windox_grep/ui/`）と COM を使うもの（`office_app.ps1`・`office_reader.ps1`）は、必要な起動口（`gui.ps1`・`convert.ps1`）が読み込む。

各スクリプト・テストからは dot-source（`. "$PSScriptRoot\lib.ps1"`）して使う。

### 5.1 パス定義

パス定義は、どのツールからも使うもの（`shared/core/paths.ps1`）と windox_grep 固有のもの（`windox_grep/core/paths_grep.ps1`）に分かれる。
...
| `$tmpDir` | `%TEMP%\windox_grep\<PID>`（プロセスごと） |
...
| `$convertPlanFile` | `$workDir\変換予定.tsv`（変換対象を数えた結果。画面が確認のダイアログに出す） |
| `$convertStartRequestFile` | `$workDir\変換開始要求`（画面が作成すると、変換処理は確認待ちから先へ進む） |
...
| `$convertPhaseScan` / `$convertPhaseConfirm` / `$convertPhaseRun` / `$convertPhaseFinish` | 変換の進み具合の段階（`準備` / `確認` / `変換` / `仕上げ`） |
| `$convertPlanColumns` | 変換予定の列名（`インデックス名` `元のフォルダ` `区分` `ファイル数` `変換対象` `新規` `更新あり` `前回未完了` `変換結果なし` `前回失敗`） |
| `$planKindConvert` / `$planKindUnchecked` / `$planKindMissing` | 変換予定の区分（`変換` / `チェックなし` / `フォルダなし`） |
...
| `writeConvertProgress` | phase, processed, remaining, failed, detail, path（既定 `$convertProgressFile`） | – | 変換の進み具合を 1 行（`<段階><TAB><処理済み><TAB><残り><TAB><失敗><TAB><いま行っていること>`）で書く。段階は `$convertPhaseScan`（準備）/ `$convertPhaseConfirm`（確認）/ `$convertPhaseRun`（変換）/ `$convertPhaseFinish`（仕上げ）。画面が読んでいる間も書けるよう共有して開き、書けなくても変換は続ける | [01_変換_4](01_変換_4_メインフローと変換一覧.md#41-メインフロー) | 変換（1 ファイルにつき 1 回） |
| `readConvertProgress` | path（既定 `$convertProgressFile`） | `@{Phase; Processed; Remaining; Failed; Detail}` / `$null` | 変換の進み具合を読む（無い・書き込みの途中なら `$null`）。画面が 1 秒ごとに呼ぶため 1 行だけ読む（数万行の変換一覧を読み直さない） | [03_画面_1](03_画面_1_インデックス管理タブ.md) | 画面 |
| `removeConvertProgress` | path（既定 `$convertProgressFile`） | – | 変換の進み具合の記録を削除する（無くてもエラーにしない） | 同上 | 変換・画面 |
| `newConvertPlanRow` | name, path, kind, total, targets, new, updated, pending, lost, failed | 変換予定の 1 行（`[pscustomobject]`） | インデックス 1 件分の変換対象の件数を作る（`$convertPlanColumns` と同じ列） | [01_変換_5](01_変換_5_変換対象の決定.md#変換予定-work変換予定tsv画面の確認に出す件数) | 変換 |
| `writeConvertPlan` | rows, path（既定 `$convertPlanFile`） | – | 変換予定を書き出す（インデックス 1 件 1 行）。画面に途中の状態を読ませないよう、一時ファイルに書いてから置き換える | 同上 | 変換 |
| `readConvertPlan` | path（既定 `$convertPlanFile`） | 変換予定の行の配列 / `$null` | 変換予定を読む（無い・列が合わなければ `$null`。件数の列は数値にする）。置き換えと重なっても読めるよう共有して開く | [03_画面_1 の 3.4](03_画面_1_インデックス管理タブ.md#34-変換の確認ダイアログ) | 画面 |
| `removeConvertPlan` | path（既定 `$convertPlanFile`） | – | 変換予定を削除する（無くてもエラーにしない） | 同上 | 変換 |
| `writeConvertStartRequest` | retryFailed, path（既定 `$convertStartRequestFile`） | – | 確認のダイアログの［変換を開始］を変換側に伝える。前回失敗したファイルも再変換するなら `$retryFailedMark` を書く | 同上 | 画面 |
| `readConvertStartRequest` | path（既定 `$convertStartRequestFile`） | `@{RetryFailed}` / `$null` | 画面からの返事を読む（まだ無ければ `$null`。変換側は返事があるまで待つ） | [01_変換_4](01_変換_4_メインフローと変換一覧.md#41-メインフロー) | 変換 |
| `removeConvertStartRequest` | path（既定 `$convertStartRequestFile`） | – | 変換開始要求を削除する（無くてもエラーにしない） | 同上 | 変換 |
...
| `getIndexStats` | rows（readStatusFile の Rows） | 名前 → `@{Total; Done; Pending; Failed; LastConverted}` | 変換一覧の行をインデックス名ごとに集計する（一覧の「ファイル」「最終変換」） | [03_画面_1](03_画面_1_インデックス管理タブ.md#31-一覧の列) | 画面（getConversionState 経由） |
...
| `newAppMutex` | name（`gui` / `convert`）, dir（既定 `$rootDir`） | `@{Mutex; Acquired}` | 同じツール（配置フォルダ）の処理を二重に動かさないための名前付きミューテックス（`Local\windox_grep_<name>_<配置フォルダの MD5>`）。`Acquired` が `$false` なら、ほかで実行中。プロセスが終われば解放されるため、強制終了しても残らない | [01_変換_4](01_変換_4_メインフローと変換一覧.md#41-メインフロー) | 変換（起動時）。画面は同じ規則の名前を自前で作る（ウィンドウを前面に出すイベントの名前と共通の鍵を使うため） |
...
変換処理（`windox_grep/convert.ps1`）と、画面の検索（`windox_grep/gui.ps1`）は次の関数を使う。

```mermaid
flowchart LR
    subgraph OTS["windox_grep/convert.ps1"]
        OT["本体"]
    end
    subgraph GRS["windox_grep/gui.ps1（検索）"]
        GR["検索・結果の出力"]
    end
    subgraph CMS["windox_grep/lib.ps1"]
        RCL["readSettings"]
...
画面（`windox_grep/gui.ps1`）は、ほかに次の関数を使う。

```mermaid
flowchart LR
    subgraph GUI["windox_grep/gui.ps1"]
        G["本体"]
    end
    subgraph CMS["windox_grep/lib.ps1"]
...
Word・PowerPoint の読み取り関数（`isZipFile` / `readDocxUnits` / `readPptxUnits` / `readXmlLines` / `writeUnits` 等）は `scripts/shared/office/office_reader.ps1` にあり、`windox_grep/convert.ps1` とテストから dot-source する（仕様は [01_変換.md](01_変換.md) の 4.4・6.4、[01_変換_2_Word.md](01_変換_2_Word.md)、[01_変換_3_PowerPoint.md](01_変換_3_PowerPoint.md)）。
