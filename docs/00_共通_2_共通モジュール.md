...
| `scripts/win_grep/core/` | win_grep のパス定義（`paths_grep.ps1`）・設定ファイル（`settings_grep.ps1`） |
| `scripts/win_grep/index/` | インデックス名の決め方（`index_name.ps1`）・インデックスの作成と集計（`index_store.ps1`） |
| `scripts/win_grep/convert/` | 変換対象の決定（`convert_plan.ps1`）・変換の状態ファイル（`convert_state.ps1`）・1 ファイルの変換（`convert_office.ps1`）・後始末（`index_migrate.ps1`） |
| `scripts/win_grep/search/` | 検索条件（`search_query.ps1`）・検索の実行（`search_run.ps1`）・元のファイルの場所（`source_map.ps1`） |
| `scripts/win_grep/ui/` | タブごとの画面（`*_tab.ps1` ほか）と、その判断層（`*_view.ps1`） |
...
依存の向きは一方向にする。**`shared/` はツール（`win_grep/`）を知らない。** ツール同士も互いを読み込まない。この決まりは `tests/meta/layers.Tests.ps1` で機械的に確かめる。

読み込み口は次の 2 つ。ファイルを足したら、どちらかから読み込む（読み込み漏れもメタテストで検出する）。

| 読み込み口 | 読み込むもの | 使う側 |
|---|---|---|
| `scripts/shared/shared.ps1` | 共通基盤（`core/`・`office/` のうち COM を使わないもの） | `win_grep/lib.ps1` |
| `scripts/win_grep/lib.ps1` | 上記＋ win_grep の `core/`・`index/`・`convert/`・`search/` | 画面・変換処理・テスト・画面が起こす別スレッド |

画面の部品（`shared/ui/`・`win_grep/ui/`）と COM を使うもの（`office_app.ps1`・`office_reader.ps1`）は、必要な起動口（`gui.ps1`・`convert.ps1`）が読み込む。

各スクリプト・テストからは dot-source（`. "$PSScriptRoot\lib.ps1"`）して使う。

### 5.1 パス定義

パス定義は、どのツールからも使うもの（`shared/core/paths.ps1`）と win_grep 固有のもの（`win_grep/core/paths_grep.ps1`）に分かれる。
...
| `newAppMutex` | name（`gui` / `convert`）, dir（既定 `$rootDir`） | `@{Mutex; Acquired}` | 同じツール（配置フォルダ）の処理を二重に動かさないための名前付きミューテックス（`Local\win_grep_<name>_<配置フォルダの MD5>`）。`Acquired` が `$false` なら、ほかで実行中。プロセスが終われば解放されるため、強制終了しても残らない | [01_変換_4](01_変換_4_メインフローと変換一覧.md#41-メインフロー) | 変換（起動時）。画面は同じ規則の名前を自前で作る（ウィンドウを前面に出すイベントの名前と共通の鍵を使うため） |
...
変換処理（`win_grep/convert.ps1`）と、画面の検索（`win_grep/gui.ps1`）は次の関数を使う。

```mermaid
flowchart LR
    subgraph OTS["win_grep/convert.ps1"]
        OT["本体"]
    end
    subgraph GRS["win_grep/gui.ps1（検索）"]
        GR["検索・結果の出力"]
    end
    subgraph CMS["win_grep/lib.ps1"]
        RCL["readSettings"]
...
画面（`win_grep/gui.ps1`）は、ほかに次の関数を使う。

```mermaid
flowchart LR
    subgraph GUI["win_grep/gui.ps1"]
        G["本体"]
    end
    subgraph CMS["win_grep/lib.ps1"]
...
Word・PowerPoint の読み取り関数（`isZipFile` / `readDocxUnits` / `readPptxUnits` / `readXmlLines` / `writeUnits` 等）は `scripts/shared/office/office_reader.ps1` にあり、`win_grep/convert.ps1` とテストから dot-source する（仕様は [01_変換.md](01_変換.md) の 4.4・6.4、[01_変換_2_Word.md](01_変換_2_Word.md)、[01_変換_3_PowerPoint.md](01_変換_3_PowerPoint.md)）。
