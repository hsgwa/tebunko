...
sequenceDiagram
    autonumber
    participant S as tebunko_grep/convert.ps1
...
- テキスト保存はセルの値しか出さないため、**図形・コメントの文字は、コピーを ZIP として直接読む**（6.7 節）。Excel で開く前に読む。旧形式（`.xls`）・パスワード付き・`.xlsb` は読まない（セルの値だけになる）。読み取りに失敗しても（ZIP が壊れている等）セルの値は変換し、`    図形・コメントを読み取れませんでした: <メッセージ>` を変換ログに記録する。
...
- 読み取る内容を増やしたため、前の版で変換したブックは、更新が無くても次の変換で変換し直す（変換一覧の「抽出版」。[01_変換_4_メインフローと変換一覧.md](01_変換_4_メインフローと変換一覧.md) 4.2）。
