...
| 対象 | 主な確認内容 |
|---|---|
| 書き方 | PowerShell class・`[pscustomobject]`・スクリプトブロックを渡す `[regex]::Replace`・`Add-Type`（`restricted/startup_check.ps1` の WPF の確認だけは許す）が無い。模擬の制限言語モードで読み込むだけでは、実行されない行は分からないため字面で調べる |
| `start.ps1` | 子プロセスの `powershell.exe` で、模擬の制限言語モード（`$ExecutionContext.SessionState.LanguageMode` を書き換える）なら制限モードで起動する旨を出して終了コード 11、FullLanguage なら 10（いつもの画面を開ける）を返す |
