...
本書は、tebunko_grep の品質をどう確かめているかをまとめる。コードを変えたときに何が自動で確かめられ、何を手で確かめるかが分かる。
...
```mermaid
flowchart LR
    C["コミット"] --> H1["pre-commit フック<br>check_commit.ps1 -Staged<br>run.ps1 -Tag Unit,Meta"]
    ...
    M --> T["v* タグ"] --> REL["release.yml<br>test を再実行して配布 zip を公開"]
```

| 関門 | 何を止めるか | 仕組み | 必須 |
|---|---|---|---|
| 単体テスト（Unit・Io・Meta） | 関数の動きの誤り、構成の決まりの違反 | `tests/run.ps1`（Pester 3.4）。[6.1](#61-単体テスト)・[6.3](#63-テストの構成と実行) | CI の `test` |
| カバレッジの下限 | テストされない処理が増えること | `tests/coverage.baseline`（今は 96.5%）を下回ると `-Ci` が失敗する。[タグと実行](#タグと実行) | CI の `test` |
...
`tests/`（Pester 3.4）で各モジュールの関数とパス定義、全スクリプトの構文、実行時コンパイル（`Add-Type -TypeDefinition`）を使わないことを検証する。テストは `scripts/` と同じ構成に並べる（[6.3](#63-テストの構成と実行)）。検索・画面の部品のテストの内容は [03_画面_6_実装とテスト_1_テスト.md](03_画面_6_実装とテスト_1_テスト.md#16-テスト) の 16.1 にある。

**共通基盤（`tests/shared/core/`・`tests/shared/office/office_files`）**
...
| フォルダ選択の部品（`getFolderEntries` など）・`testOfficeFile` | [03_画面_6_実装とテスト_1_テスト.md](03_画面_6_実装とテスト_1_テスト.md#16-テスト) の 16.1 |

**設定ファイル（`tests/tebunko_grep/core/settings`・`tests/shared/core/settings`）**

| 対象 | 主な確認内容 |
|---|---|
| `writeSettingsFile` / `readSettingsData` / `mergeSettingValues` / `toSettingValue`（`tests/shared/core/settings.Tests.ps1`） | 書く側が知らないキー（ほかのツールの設定）を残す、使わなくなったキー（removeKeys）を消す、JSON として読めないファイルは書く側の設定だけで書き直す、無い・空・null の中身は `$null`、読めなければファイル名入りの例外、既定値と同じ型にそろえる（配列・文字列・真偽値・数値・入れ子）、記載の無い・null のキーは既定値 |
...
**インデックス名と TSV の名前（`tests/tebunko_grep/index/index_name`）**
...
**インデックスの管理（`tests/tebunko_grep/index/index_store`）**
...
**インデックス作成（`tests/tebunko_grep/indexer/`）**
...
| `readStatusLines` / `renameStatusIndexName` / `removeStatusIndexName` | 行の順序と内容を保った書き換え、以前の形式（抽出版の列が無い）の取り込み一覧 |
| `readIngestingFile` / `writeIngestingFile` / `removeIngestingFile` | 回数と相対パスの往復、無い・壊れていれば `$null` |
...
| `createTargetList` / `findOfficeFiles` / `waitForIndexingApproval` | 新規・更新あり・前回未完了・インデックスなし・前の抽出版は取り込み対象、更新の無い取り込み済み・前回失敗は対象外、元のファイルが無くなったらインデックスと行を消す、アクセスできないフォルダがあったときは行とインデックスを残す、画面の返事を待つ |
| `publishTsv` / `removeStaleTmpDirs` / `moveLegacyIndex` / `removeDroppedFolders` / `migrateFlatIndex` | 作業フォルダの TSV をインデックスへ移す（`[` `]`・260 文字超のパス）、終了したプロセスの作業フォルダだけを削除する、以前の形式のインデックスの移行（途中で止まっても続きから移す） |
| `indexer.ps1`（起動口） | 設定とインデックスを `$TestDrive` に差し替えて通しで動かす（Excel は使わず、`.docx`・`.pptx` を取り込む）。続けられないエラーと終了コード、2 回目は更新の無いファイルを取り込まない、`-RetryFailed`、強制終了したファイルを最後に回す・続けて強制終了したら失敗にする、画面の確認での取りやめ・中止（終了コード 2）、制限時間、取り込み中に元のファイル・フォルダが無くなった場合 |

**元のファイルの特定（`tests/tebunko_grep/search/source_map`）**
...
**比較（`tests/tebunko_diff/`）**

| テスト | 主な確認内容 |
|---|---|
| `tests/tebunko_diff/diff/diff_core.Tests.ps1` | 行の差分（一致する行の対応づけ）、似ている行の組み合わせ（1 行先との比べ方）、文字単位の違い、同じ行をたたむ。大きな差分の速さはタグ `Slow` |
| `tests/tebunko_diff/diff/diff_office.Tests.ps1` | Excel（行とセル・列の追加・削除・移動・シート名の変更・行の移動・空の行と列）・Word（段落・移動）・PowerPoint（スライド・ノート・並べ替え）の比べ方、場所の順番の合わせ方、大きすぎる場所 |
...
| `tests/tebunko_diff/core/settings_diff.Tests.ps1` | 比較の設定の既定値と往復、知らない値の補正、検索の設定のキーを残す、比較の設定に無いキーは例外、比較元・比較先のパスを保存しない |
...
| `check_signoff.ps1` | 作者の `Signed-off-by` があれば通す、無い・作者と違うメールアドレスなら止める、コメント行と `git commit -v` の差分の中は数えない、マージコミット・bot は調べない、範囲より前のコミットは調べない |
...
**COM を使う処理（`tests/shared/office/office_extract`・`tests/shared/office/office_app`）**

Excel・Word・PowerPoint の COM を呼ぶ処理（`shared/office/office_extract.ps1`・`shared/office/office_app.ps1`。インデクサと比較の抽出プロセスが使う）は、Office を使わずに流れを検証する。COM の入口は `getApp`（アプリの取得）と `New-Object -ComObject` だけなので、ここを Pester の `Mock` で差し替え、本物と同じ呼び方ができる偽のオブジェクト（呼ばれたメソッドと引数を記録し、保存では本物と同じ形式のファイルを書く）を返す。実機での動作は 6.2 の手動の結合テストで確かめる。

| テスト | 主な確認内容 |
|---|---|
| `tests/shared/office/office_extract.Tests.ps1` | 表示しているシートだけを書き出す、作業フォルダのコピーを読み取り専用・ダミーのパスワードで開いて保存せずに閉じる、使用範囲の左上を A1 からの位置に戻す、使用範囲が膨らんだシートは一時シートにコピーして書き出して消す（構成が保護されていれば元のシートのまま）、新形式のブックの図形・コメントを別の場所の TSV にする（読めなくてもセルの値は取り込む）、新形式の Word は Word を使わずに読む、旧形式の Word・PowerPoint を新形式に保存してから読む、壊れた `.pptx` を PowerPoint に渡さない、失敗しても文書を閉じて作業ファイルを消す |
| `tests/shared/office/office_app.Tests.ps1` | 起動時の設定（非表示・警告なし・イベントなし・リンクを更新しない・マクロ無効。PowerPoint は `Visible` を変えない）、起動済みのアプリを使い回す、自分で起動したアプリだけを制限時間の強制終了の対象にする、Quit して終わらなければ強制終了する（Excel は 1 秒、Word・PowerPoint は 5 秒待つ）、利用者のアプリ・利用者が文書を開いたアプリは終了させない、制限時間の監視 |

**画面の部品（`tests/tebunko_grep/ui/`・`tests/shared/ui/`）**

判断層（`*_view.ps1`）はそのままテストする。`result_list.ps1`・`open_source.ps1`・`preview.ps1`・`index_tree.ps1` は `$ui`・`$window` を偽物にし、Excel・エクスプローラーの起動は `Mock` して実際には開かない。クリップボードは利用者の PC のものを書き換えるため、「コピーしない場合」だけを確かめる。型（`types.ps1`・`types_grep.ps1`）はプロパティの変更通知と各メソッドを確かめる。内容は [03_画面_6_実装とテスト_1_テスト.md](03_画面_6_実装とテスト_1_テスト.md#16-テスト) の 16.1。
...
タグ `Meta` で、[04_安全性.md](04_安全性.md) の主張を機械的に検証する。導入審査で「危険な処理・ライブラリを使っていないこと」を示すための検査であり、将来の変更でこの前提が崩れた場合に失敗する。スクリプトを読むだけで動作するため、Office もテストデータも要らない（33 件）。
...
| フォルダ | 内容 |
|---|---|
| `tests/helpers/` | 共通の準備（`load.ps1`。`$here`・`${scriptsDir}`・`${testDataDir}` を決めて `tebunko_grep/lib.ps1` を読み込む）とテスト用の TSV 作成（`tsv.ps1`） |
...
| `tests/tebunko_grep/indexer/` | `indexer_state`・`indexer_decide`・`indexer_plan`・`index_migrate`、起動口の通しのテスト（`indexer`） |
| `tests/tebunko_grep/search/` | `search_query`・`search_run`・`source_map` |
| `tests/tebunko_grep/ui/` | 画面の判断層（`index_view`・`indexing_view`・`search_view`・`preview_view`）と、`$ui` を偽物にした画面の部品（`result_list`・`open_source`・`preview`・`index_tree`）・型（`types_grep`） |
...
| `tests/tools/` | 開発用の道具（`check_commit_message`・`check_signoff`・`check_markdown_links`） |
...
| テスト | 確かめること |
|---|---|
| `structure` | `$rootDir` などのパス定義、全 `.ps1` の構文、`Add-Type -TypeDefinition` を使わない、`ui/` のスクリプトで `$PSScriptRoot` を使わない、画面が指すインデクサのファイルがある、XAML が XML として読める・`x:Name` がある（ウィンドウの枠 `tebunko/xaml/tebunko.xaml` と、各タブの XAML の名前が `tebunko/gui.ps1` の一覧と合う）、型を読み込む順（shared → tebunko_grep → tebunko_diff）で継承が解決できる |
| `encoding` | 全 `.ps1`・`.xaml` が BOM 付き UTF-8 で、改行が CRLF。`.github/codecov.yml` が ASCII の文字だけ |
| `layers` | `shared/` にツールの名前が出てこない、ツール同士が互いを読み込まない（両方を読み込んでよいのは `tebunko/` だけで、ツールから `tebunko/` も読み込まない）、起動口（`tebunko/gui.ps1`・`tebunko_grep/indexer.ps1`・`tebunko_diff/differ.ps1`）からたどれない `.ps1` が無い、判断層（`text.ps1`・`index_name.ps1`・`search_query.ps1`・`indexer_decide.ps1`・`*_view.ps1`・`tebunko_diff/diff/*.ps1`）に画面への依存が無い |
...
| `safety` | 危険な処理を使っていない、起動用バッチ（`tebunko.bat`）が `RemoteSigned` で `Bypass` を使わない、Office をマクロ無効・読み取り専用で開く、原本を書き換えない、書き込み先が `work`・`%TEMP%` だけ、PSScriptAnalyzer の指摘が 0 件、審査用の資料がそろっている（6.1、[04_安全性.md](04_安全性.md)） |
...
- 対象は `scripts/` の `.ps1` のうち、画面層の `gui.ps1`・`*_tab.ps1`・`shell.ps1`・`app_host.ps1`・`*_dialog.ps1`・`grep_page.ps1`・`diff_pane.ps1`・`diff_tree.ps1` を除いたもの（`tests/run.ps1` の `CodeCoverage` の条件）。除いたものは自動テストの対象外で、手で確かめる。画面層のファイルを足したら、この条件から外れているか（分母に入っていないか）を確かめる。
- 値は Pester のコマンド単位（実行されたコマンドの数 ÷ 全コマンドの数。小数点以下 1 桁）。
- **下限は `tests/coverage.baseline`（今は 96.5）。** `-Ci` はこれを下回ると失敗し、CI の必須チェック `test` が通らない。下回ったらテストを足して戻し、下限は下げない。上がったら、同じ PR で下限を実測の値まで上げる。
- 全体の目標値は決めない。判断層は 95% 以上を保ち、数字を上げるためだけのテスト（結果を確かめないもの）は書かない（AGENTS.md「テストカバレッジの方針」）。
- `-Ci` はカバレッジを Cobertura 形式の XML（`work\test\coverage.xml`）にも書き出す。Pester 3.4 はカバレッジをファイルに出せないため、`tests/run.ps1` が結果（コマンド単位）から行単位に組み立てる。1 行に実行されたコマンドが 1 つでもあれば、その行は通ったものとする。そのため、行単位の値（Codecov の表示）はコマンド単位の値と少し違う。ファイル名はリポジトリからの相対パスで書き、利用者名を含む絶対パスは入れない。
...
| `release.yml` | `test`・`release` | `v` で始まるタグの push | テストのうえ、配布 zip を GitHub Release に載せる | – |
...
`v` で始まるタグを push すると動き、`test.yml` と同じ検査・テストを通したうえで、`tools/new_release_package.ps1` で配布 zip（`tebunko-<タグ>.zip`。展開すると `tebunko/` フォルダになる）を作って GitHub Release に載せる。

- zip にはカタログ（`tebunko.cat`）とハッシュ一覧（`SHA256SUMS.txt`）を同梱し（[04_安全性.md](04_安全性.md) の 5.1）、ハッシュ一覧はリリースにも載せる。zip 自体の SHA256 はリリースの説明に書く（同 5.6 の VirusTotal での照会用）
- zip のビルドの来歴を Sigstore で署名して GitHub に登録し、署名の bundle（`tebunko-<タグ>.zip.sigstore.json`）もリリースに載せる（同 5.1）
...
| `pre-commit` | 2 | `tests/run.ps1 -Tag Unit,Meta -Quiet`：速いテスト（`Io` を含む既定のテストと静的解析は CI で行う）。失敗したテストだけを表示する | 1 分強 |
