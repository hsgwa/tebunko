...
| カバレッジの下限 | テストされない処理が増えること | `tests/coverage.baseline`（今は 96.6%）を下回ると `-Ci` が失敗する。[タグと実行](#タグと実行) | CI の `test` |
...
- **下限は `tests/coverage.baseline`（今は 96.6）。** `-Ci` はこれを下回ると失敗し、CI の必須チェック `test` が通らない。下回ったらテストを足して戻し、下限は下げない。上がったら、同じ PR で下限を実測の値まで上げる。
