...
| 制限モードが使う関数 | `tests/meta/clm_cases.ps1` の呼び出し（設定の読み書き・インデックス名・TSV の名前と場所・検索の条件・元のファイルの場所・パスの正規化・ファイルの書き込み・インデックスの作成）を、FullLanguage と模擬の制限言語モードの子プロセスで動かし（`tests/meta/clm_run.ps1`）、結果（JSON。ファイルはバイト列）が同じ。インデックスの作成（`restrictedIndexing`）は、Word・PowerPoint を実際に取り込み、できた TSV のバイト列まで比べる |
