...
| カバレッジの下限 | テストされない処理が増えること | `tests/coverage.baseline`（今は 97.1%）を下回ると `-Ci` が失敗する。[タグと実行](#タグと実行) | CI の `test` |
...
| 制限言語モードでの動作 | 制限モードに使う部品が、本物の制限言語モード（WDAC）で動かなくなること | `tools/clm/check.ps1`。[CI](#ci) の `clm.yml` | ―（CI の `clm`） |
...
**設定ファイル（`tests/tebunko_grep/core/settings`）**
...
**COM を使う処理（`tests/tebunko_grep/indexer/extract_office`・`tests/shared/office/office_app`）**
...
| テスト | 主な確認内容 |
|---|---|
| `tests/tebunko_grep/indexer/extract_office.Tests.ps1` | 表示しているシートだけを書き出す、作業フォルダのコピーを読み取り専用・ダミーのパスワードで開いて保存せずに閉じる、使用範囲の左上を A1 からの位置に戻す、使用範囲が膨らんだシートは一時シートにコピーして書き出して消す（構成が保護されていれば元のシートのまま）、新形式のブックの図形・コメントを別の場所の TSV にする（読めなくてもセルの値は取り込む）、新形式の Word は Word を使わずに読む、旧形式の Word・PowerPoint を新形式に保存してから読む、壊れた `.pptx` を PowerPoint に渡さない、失敗しても文書を閉じて作業ファイルを消す |
...
**制限言語モードで動かすファイルの検査（`tests/meta/clm.Tests.ps1`）**

タグ `Meta` で、制限モード（`scripts/tebunko_grep/start.ps1` と、そこから dot-source でたどれるファイル）が制限言語モードでも動くことを確かめる。本物の制限言語モード（WDAC）での確認は CI の `clm.yml`（[CI](#ci)）が受け持つ。

| 対象 | 主な確認内容 |
|---|---|
| 書き方 | PowerShell class・スクリプトブロックを渡す `[regex]::Replace`・`Add-Type`（`restricted/startup_check.ps1` の WPF の確認だけは許す）が無い。`restricted/` と `start.ps1` には `[pscustomobject]` も無い（いつもの画面と共用のファイルは、画面だけが使う関数で使ってよい）。模擬の制限言語モードで読み込むだけでは、実行されない行は分からないため字面で調べる |
| `start.ps1` | 子プロセスの `powershell.exe` で、模擬の制限言語モード（`$ExecutionContext.SessionState.LanguageMode` を書き換える）なら制限モードで起動する旨を出し、制限モードの部品（`lib_restricted.ps1`）を読み込んで終了コード 11、FullLanguage なら 10（いつもの画面を開ける）を返す |
| 制限モードが使う関数 | `tests/meta/clm_cases.ps1` の呼び出し（設定の読み書き・インデックス名・TSV の名前と場所・検索の条件・元のファイルの場所・パスの正規化・ファイルの書き込み・インデックスの作成）を、FullLanguage と模擬の制限言語モードの子プロセスで動かし（`tests/meta/clm_run.ps1`）、結果（JSON。ファイルはバイト列）が同じ。インデックスの作成（`restrictedIndexing`・`xlsxCells`）は、Word・PowerPoint・Excel を実際に取り込み、できた TSV のバイト列まで比べる |

模擬の制限言語モードは本物より厳しい（Microsoft が署名したモジュールの `Expand-Archive`・`Start-Job` まで止まる）。模擬で動けば本物でも動く。
...
| `tests/shared/core/` | `fs`・`text`・`folder` |
...
| `tests/tebunko_grep/indexer/` | `indexer_state`・`indexer_decide`・`indexer_plan`・`extract_office`・`index_migrate`、起動口の通しのテスト（`indexer`） |
...
| テスト | 確かめること |
|---|---|
| `structure` | `$rootDir` などのパス定義、全 `.ps1` の構文、`Add-Type -TypeDefinition` を使わない、`ui/` のスクリプトで `$PSScriptRoot` を使わない、`gui.ps1` が指すインデクサのファイルがある、XAML が XML として読める・`gui.ps1` が使う `x:Name` がある、型を読み込む順で継承が解決できる |
...
- **下限は `tests/coverage.baseline`（今は 97.1）。** `-Ci` はこれを下回ると失敗し、CI の必須チェック `test` が通らない。下回ったらテストを足して戻し、下限は下げない。上がったら、同じ PR で下限を実測の値まで上げる。
...
| `clm.yml` | `clm (windows-2022)`・`clm (windows-2025)` | PR・main への push（スクリプト・`tools/clm/`・起動用バッチが変わったとき） | WDAC のポリシーを当てて本物の制限言語モードにし、部品が見込みどおりに動くかを確かめる | – |
...
AppLocker・WDAC の PC では、許可されていないスクリプトが制限言語モード（Constrained Language Mode）で動く。制限モード（Issue #71）に使う部品が、そこで思ったとおりに動くかを確かめる。手元の開発機（Windows 11 Home）では本物の制限言語モードを作れず、スクリプトの中で `$ExecutionContext.SessionState.LanguageMode` を書き換える模擬は本物と挙動が違う（Microsoft が署名したモジュールの `Expand-Archive`・`Start-Job`・`Scripting.FileSystemObject` まで止まる）ため、ここで確かめる。

- 使い捨てのランナーに `tools/clm/enable_wdac.ps1` で WDAC のポリシーを当てる。Windows に付いている例（`DefaultWindows_Enforced.xml`）を元にし、ランナーが使う `C:\` のフォルダの exe だけをパスの規則で足す。windows-2022 は WMI で、windows-2025 は `CiTool` で、再起動なしに当てる
- 当てた後に新しい `powershell.exe` で `tools/clm/check.ps1` を動かす。項目ごとに「動く」か「止まる」かの見込みを書いてあり、見込みと違う項目があれば失敗にする。結果はジョブの概要（Summary）に表で出る
- 当てた後の手順は `shell: cmd` にする。`shell: powershell` は一時フォルダの ps1 を動かすため、手順そのものが制限言語モードになる
- 必須チェックにはしない。ランナーの版が上がると、ポリシーの当て方が変わるおそれがあるため
- 確かめたことの例: `tar.exe` は引数を ANSI のコードページ（日本語版の Windows は CP932、英語版のランナーは CP1252）で受け取るため、そこに無い文字を含むパスは引数では開けない（英語版では日本語の名前も開けない）。cmd のリダイレクト（`tar -xf - < "<パス>"`）なら開ける。`[regex]::Replace` にスクリプトブロックを渡すと、エラーにならずにブロックの文字列そのものに置き換わる
- AppLocker でも試したが、規則の判定は効いても PowerShell が制限言語モードにならなかったため使わない

**`release.yml`（配布物の公開）**

`v` で始まるタグを push すると動き、`test.yml` と同じ検査・テストを通したうえで、`tools/new_release_package.ps1` で配布 zip（`tebunko_grep-<タグ>.zip`）を作って GitHub Release に載せる。
...
- zip のビルドの来歴を Sigstore で署名して GitHub に登録し、署名の bundle（`tebunko_grep-<タグ>.zip.sigstore.json`）もリリースに載せる（同 5.1）
