...
**スレッドとプール（`tests/shared/core/worker_pool`・`tests/tebunko_grep/search/search_service`）**
...
| `tests/tebunko_grep/indexer/` | `indexer_state`・`indexer_decide`・`indexer_plan`・`extract_office`・`index_migrate`・`indexing_session`、起動口の通しのテスト（`indexer`） |
| `tests/tebunko_grep/search/` | `search_query`・`search_run`・`pack_search`・`search_service`・`source_map` |
