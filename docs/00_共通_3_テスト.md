...
| 設計書のリンク | リンク先のファイル・見出しが無いこと | `mkdocs build --strict` | CI の `docs` |
...
| `tests/tools/` | 開発用の道具（`check_commit_message`・`check_signoff`） |
| `tests/meta/` | 構成を守るテスト（`structure`・`encoding`・`layers`・`runner`）と安全性の検査（`safety`） |
