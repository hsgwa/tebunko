...
| `createTargetList` / `findOfficeFiles` / `waitForIndexingApproval` | 新規・更新あり・前回未完了・インデックスなし・前の抽出版は取り込み対象、更新の無い取り込み済み・前回失敗は対象外、元のファイルが無くなったらインデックスと行を消す（集約ファイルから外すよう `Removed` で返す）、アクセスできないフォルダがあったときは行とインデックスを残す、画面の返事を待つ |
...
| `indexer.ps1`（起動口） | 設定とインデックスを `$TestDrive` に差し替えて通しで動かす（Excel は使わず、`.docx`・`.pptx` を取り込む）。続けられないエラーと終了コード、2 回目は更新の無いファイルを取り込まない、`-RetryFailed`、強制終了したファイルを最後に回す・続けて強制終了したら失敗にする、画面の確認での取りやめ・中止（終了コード 2）、制限時間、取り込み中に元のファイル・フォルダが無くなった場合、元のファイルが無くなったら次の作成で集約ファイルから外す、前回残った TSV を次の作成の始めに集約ファイルへ入れる、中止しても取り込んだ分は集約ファイルとシステムインデックスに入れる |
...
| `tests/tebunko_grep/index/` | `index_name`・`index_store`・`pack_format` |
| `tests/tebunko_grep/indexer/` | `indexer_state`・`indexer_decide`・`indexer_plan`・`extract_office`・`index_migrate`、起動口の通しのテスト（`indexer`） |
| `tests/tebunko_grep/search/` | `search_query`・`search_run`・`pack_search`・`source_map` |
| `tests/tebunko_grep/ui/` | 画面の判断層（`index_view`・`indexing_view`・`search_view`・`preview_view`・`settings_view`）と、`$ui` を偽物にした画面の部品（`result_list`・`open_source`・`preview`・`index_tree`）・型（`types_grep`） |
...
手動で起動し、同じ量のデータで pack の作成・検索の速さと、そのあいだのリソース（メモリ・CPU・スレッド・ハンドル・GC）を測る。作り替えの前後などで数字を見るときに使う。マージは止めない。

```
gh workflow run perf.yml -f ref=<測る ref> -f scale=0.1
```

- 入力: `ref`（測る ref。作業中のブランチも測れる）、`scale`（データの量。`1` でブック 5.4 万・TSV 16 万・約 1GB）、`repeat`（検索を繰り返す回数。中央値を出す）、`words`（足して検索する語）
- 道具（`tools/measure_perf.ps1`）はワークフローの ref から、測るコードは入力の ref から取り出す。道具が呼ぶ関数（`publishIndexFolders`・`getIndexPackFiles`・`searchPackIndex`）が無い ref（pack 形式より前の版）は、はっきりしたメッセージで止まる
- データは [tebunko-perfdata](https://github.com/hsgwa/tebunko-perfdata) の `new_index.ps1` で毎回作る（取り込みの一時置き場と同じ形の TSV）。版は `perf.yml` の `PERFDATA_SHA` でコミットに固定する。検索する語は同じリポジトリの `words.tsv`（0 件・まれ（3 件）・大量・正規表現）
- 測るもの
  - pack の作成 … インデクサと同じ `publishIndexFolders`（pack を書く・TSV を消す・システムインデックスを作る）。Office の取り込みは、ランナーに Office が無いため測らない
  - 検索 … 画面の検索スレッドと同じ流れ（新しい Runspace で `lib.ps1` を読む → 列挙 → 照合。上限 1 万件）。読んだ内容の入れ物を新しくしてから初回・2 回目を行い、これを `repeat` 回繰り返す。高速検索は、ランナーに Windows Search が無いため使わない
  - リソース … 別のスレッドで 0.2 秒ごとと段階の境目に、このプロセスのワーキングセット・プライベート・マネージドのメモリ、CPU 時間、スレッド・ハンドルの数、GC の回数と、PC 全体の CPU・空きメモリを記録する
- 結果は、ジョブの Summary（表と Mermaid のグラフ）と、artifact `perf-<実行の番号>`（`summary.md`・`result.json`・`resource.csv`。90 日残す）に出す。パス・ファイル名は出さない
- ランナー（4 コア）は手元の PC と条件が違う。Defender のリアルタイム保護の状態は結果の「環境」に出す。比べるときは、同じワークフローで測った数字どうしで比べる
- 手元でも同じ道具で測れる（`.\tools\measure_perf.ps1 -Index <TSV のフォルダ> -Work <作業フォルダ> -Words <words.tsv>`）。渡した TSV は pack に書き換えて消える
