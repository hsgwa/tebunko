...
| `getPackFileName` / `splitPackBooksByExtension` / `encodePackValue` / `decodePackValue` / `convertToPackBody` | まとめファイルの名前、拡張子ごとの分け方、値の `%XX` の往復、中身の改行を LF にそろえ U+001C〜U+001F を除く |
