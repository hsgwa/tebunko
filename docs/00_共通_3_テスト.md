...
- 確かめたことの例: `tar.exe` は引数を CP932 で受け取るため、CP932 に無い文字（`𠮷` や絵文字）を含むパスは引数では開けず、cmd のリダイレクト（`tar -xf - < "<パス>"`）なら開ける。`[regex]::Replace` にスクリプトブロックを渡すと、エラーにならずにブロックの文字列そのものに置き換わる
