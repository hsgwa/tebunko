...
- zip にはツール本体（`tebunko.bat`・`scripts/`）と `LICENSE` だけを入れる。カタログ（`tebunko.cat`）・ハッシュ一覧（`SHA256SUMS.txt`）・部品表（`sbom.cdx.json`）は zip と並べてリリースに載せ（[04_安全性.md](04_安全性.md) の 5.1）、README・SECURITY はリリースの説明からリンクする。zip 自体の SHA256 はリリースの説明に書く（同 5.6 の VirusTotal での照会用）
