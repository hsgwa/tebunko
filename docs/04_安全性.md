...
| 場所 | 起動するもの | 用途 |
|---|---|---|
| `tebunko/ui/index_tab.ps1:615`・`620`、`tebunko/ui/open_source.ps1:311` | `explorer.exe` | 一覧・検索結果から元のファイルの場所を開く（利用者の操作時のみ） |
| `tebunko/ui/indexing_tab.ps1:196` | `powershell.exe` | 本ツール自身のインデクサ `tebunko/indexer.ps1` を起動（`-NoProfile -ExecutionPolicy RemoteSigned -WindowStyle Hidden`） |
| `tebunko/gui.ps1:309` | `powershell.exe` | 本ツール自身の画面 `tebunko/gui.ps1` を開き直す（［8 設定］でワークスペースを変えたときだけ。`-NoProfile -STA -ExecutionPolicy RemoteSigned -WindowStyle Hidden`） |
