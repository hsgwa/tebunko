...
|---|---|---|---|
| 構成物 | Windows PowerShell スクリプト（`scripts/**/*.ps1`）、画面定義（`*.xaml`）、起動用 `tebunko.bat`、アイコン `tebunko_grep.ico` のみ。実行可能バイナリ（`.exe` / `.dll`）を同梱しない | 配布 zip の中身は `tools/new_release_package.ps1` が決める | [5.1](#51-配布物の完全性カタログハッシュ一覧来歴の署名) |
...
`tebunko.bat` は、画面を起動する前に、配布フォルダの `scripts` 配下の Mark-of-the-Web を解除し、そのあと画面を起動する。
...
| 項目 | 値 |
|---|---|
| 対象ファイル数 | 52（`scripts/` 配下すべてと `tebunko.bat`） |
