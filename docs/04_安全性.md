...
| Windows Search | **読み取りだけ**。高速検索（[02_検索.md](02_検索.md) 4.4）で、OLE DB（`Search.CollatorDSO`）に SELECT だけを送る（`windows_search.ps1` の 1 か所。管理者権限は要らない）。Windows Search の設定（索引の対象など）もファイルのアクセス権も変えない。システムインデックスを作る間は、自分のプロセスの優先度を下げる（BelowNormal） | `safety.Tests.ps1`「Windows Search への問い合わせは windows_search.ps1 だけで行い、SELECT だけを送る」 | [2.1](#21-検査項目と結果) |
...
| 書き込み先 | `work/` 配下（既定は `%USERPROFILE%\Documents\tebunko`。利用者が画面で選んだフォルダ（ワークスペース）にも置ける）、`%TEMP%\tebunko_grep\<PID>` 配下、`setting.config`（ツールのフォルダに書き込めなければ `%LOCALAPPDATA%\tebunko\<鍵>`）、利用者が指定した検索結果の出力先のみ | `safety.Tests.ps1` | [3.1](#31-書き込み削除する場所) |
...
| 場所 | 読み込むアセンブリ | 用途 |
|---|---|---|
| `tebunko_grep/gui.ps1:8` | `PresentationFramework` / `PresentationCore` / `WindowsBase` / `System.Windows.Forms` | 画面（WPF）の表示、フォルダ選択（WinForms） |
...
| `tebunko_grep/gui.ps1:309` | `powershell.exe` | 本ツール自身の画面 `tebunko_grep/gui.ps1` を開き直す（［8 設定］でワークスペースを変えたときだけ。`-NoProfile -STA -ExecutionPolicy RemoteSigned -WindowStyle Hidden`） |
...
| 場所 | 内容 | 定義 |
|---|---|---|
| `work/` 配下（既定は `%USERPROFILE%\Documents\tebunko`。利用者が画面で選んだフォルダ（ワークスペース）にも置ける） | TSV インデックス（`work/index/`）、取り込み一覧・インデックス作成ログ・制御用ファイル、検索結果 | `shared/core/data_dir.ps1:41`、`tebunko_grep/core/paths_grep.ps1:12`・`13`・`18`・`26`〜`41` |
| `%TEMP%\tebunko_grep\<PID>` 配下 | 取り込みの作業領域（原本のコピー・中間 TSV）。インデックス作成の完了時・開始時に空にする | `tebunko_grep/core/paths_grep.ps1:8` |
| `setting.config`（ツールを置いたフォルダの直下） | 画面が保存する設定（クロール対象フォルダ・検索対象インデックス・`work` の置き場所など） | `shared/core/data_dir.ps1:40`、`tebunko_grep/core/settings_grep.ps1:5` |
...
| 項目 | 値 |
|---|---|
| 対象ファイル数 | 52（`scripts/` 配下すべてと `tebunko_grep.bat`） |
...
| 指標 | 値 |
|---|---|
| テスト件数 | 1,160 件（既定のタグ Unit・Io・Meta。失敗 0 件。うち安全性・構成の検査（タグ `Meta`）が 116 件） |
| コードカバレッジ | 97.1%（5,701/5,873 コマンド。手元の実測。CI では本物の Windows Search を使うテストが保留になるため 96.9%）。対象は画面の起動口・タブ・ダイアログを除くスクリプト（`tests/run.ps1` の `CodeCoverage` の条件）。Office の COM を使うインデクサは、COM の入口（`getApp`）を偽のオブジェクトに差し替えて検証している。検索結果の一覧・プレビューなど計測の対象に入る画面の部品は、画面のコントロールを偽のオブジェクトに差し替えて検証している。下限は `tests/coverage.baseline`（96.5%）で、下回ると CI が失敗する |
