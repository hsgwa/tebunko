...
|---|---|---|---|
| 構成物 | Windows PowerShell スクリプト（`scripts/**/*.ps1`）、画面定義（`*.xaml`）、起動用 `tebunko.bat`、アイコン `tebunko.ico` のみ。実行可能バイナリ（`.exe` / `.dll`）を同梱しない | 配布 zip の中身は `tools/new_release_package.ps1` が決める | [5.1](#51-配布物の完全性カタログハッシュ一覧来歴の署名) |
...
| 管理者権限・常駐 | **不要・しない**。昇格要求・サービス登録・タスクスケジューラ登録・自動起動の登録を行わない。画面を閉じれば終了する | `safety.Tests.ps1` | [2.1](#21-検査項目と結果) |
| レジストリ | **読み書きしない** | `safety.Tests.ps1` | [2.1](#21-検査項目と結果) |
...
| 取り込み・比較の対象のファイル | **読み取りのみ**。原本を開かず、作業フォルダへコピーしたものを読む | `safety.Tests.ps1` | [3.2](#32-取り込み比較の対象のファイルは書き換えない) |
...
| 書き込み先 | `work/` 配下、`%TEMP%\tebunko_grep\<PID>` 配下、`%TEMP%\tebunko\diff\<PID>` 配下（比較の作業フォルダ）、`setting.config`、利用者が指定した検索結果・比較結果の出力先のみ | `safety.Tests.ps1` | [3.1](#31-書き込み削除する場所) |
| 配布物の完全性 | 配布 zip にカタログ（`tebunko.cat`）とハッシュ一覧（`SHA256SUMS.txt`）を同梱し、zip にはビルドの来歴の署名を付ける | `release.yml` | [5.1](#51-配布物の完全性カタログハッシュ一覧来歴の署名) |
...
| 規模（監査の目安） | `scripts/` 配下の `.ps1` 60 ファイル・14,498 行（空行を除く）・509 関数。第三者依存が無いため、監査対象はこの範囲で閉じる | – | [5.7](#57-同梱の機械検査と品質の指標) |
...
検査対象は `scripts/` 配下のすべての `.ps1`（60 ファイル）である。下の確認コマンドは、説明のコメント行（「〜は使わない」などの記述）を除いて検索する。「該当 0 件」は、コマンドが 1 件も出力しないことを意味する。同じ検査を `tests/meta/safety.Tests.ps1` の「危険な処理を使っていないこと」が行い、CI（`test.yml` の `test`。main へのマージに必須）で PR ごとに実行する。
...
| 11 | 外部プロセスの起動 | 5 か所のみ（2.3） | `scan 'Start-Process'` |
...
実行時コンパイル（`csc.exe` の起動）を使わないのは設計方針である。検索処理・画面の型定義などは C# を使わず PowerShell と .NET の直接呼び出しで書いている（[03_画面_6_実装とテスト.md 12.2](03_画面_6_実装とテスト.md#122-実行時コンパイルcscexeを使わない)）。実行時に一時フォルダへコンパイル結果を書き出す動作が無いため、EDR やアプリケーション制御（AppLocker・WDAC）との衝突も避けられる。`tests/meta/structure.Tests.ps1` の「実行時コンパイル（csc.exe）を使わない」も、`Add-Type -TypeDefinition` が無いことを確かめる。

### 2.2 Office ファイルを開くときの設定

インデックス作成と比較のために Excel・Word・PowerPoint を COM で操作するが、アプリを起動したときに次の設定を行う（`shared/office/office_app.ps1` の `getApp`）。比較の抽出プロセス（`tebunko_diff/differ.ps1`）も、インデクサと同じ `shared/office/office_extract.ps1`・`office_app.ps1` で開くため、設定は同じである。`tests/meta/safety.Tests.ps1` の「Office ファイルを安全に開くこと」がこれを確かめる。
...
| `Open` の `ReadOnly` | 真 | 読み取り専用で開く（`shared/office/office_extract.ps1:124`（Excel）・`204`（Word）・`230`（PowerPoint）） |

パスワード付きファイルは、開くときに固定文字列 `"dummy"` をパスワードとして渡す（同じ 3 行）。これはパスワードを破るための処理ではなく、**パスワード入力ダイアログを出さずに確実に失敗させる**ための指定である。パスワード付きファイルは取り込まれず、`work\取り込み一覧.tsv` に失敗として記録される（比較では「比較できない」と表示する）。本ツールがパスワードを入力・保存・送信することはない。
...
| 場所 | 起動するもの | 用途 |
|---|---|---|
| `tebunko_grep/ui/index_tab.ps1:617`・`622`、`tebunko_grep/ui/open_source.ps1:202` | `explorer.exe` | 一覧・検索結果から元のファイルの場所を開く（利用者の操作時のみ） |
| `tebunko_grep/ui/indexing_tab.ps1:196` | `powershell.exe` | 本ツール自身のインデクサ `tebunko_grep/indexer.ps1` を起動（`-NoProfile -ExecutionPolicy RemoteSigned -WindowStyle Hidden`） |
| `tebunko_diff/ui/diff_tab.ps1:465`（`launchDiffer`） | `powershell.exe` | 本ツール自身の比較の抽出プロセス `tebunko_diff/differ.ps1` を起動（`-NoProfile -ExecutionPolicy RemoteSigned -WindowStyle Hidden -File ...\differ.ps1 -JobDir <比較の作業フォルダ>`。利用者が［比較］を押したときのみ） |

元のファイルを開く操作（`shared/ui/open_file.ps1:21` の `openWithShell`）では、`Start-Process` ではなく `ProcessStartInfo` を使う（`Start-Process` は `[` `]` を含むパスをワイルドカードとして解釈するため）。起動対象は、利用者が選んだ行のファイルと、それに関連付けられたアプリケーションである。
...
削除（`Remove-Item`）は 18 か所あるが、対象はすべて `work/` 配下・`%TEMP%\tebunko_grep\<PID>` 配下・`%TEMP%\tebunko\diff` 配下、すなわち**本ツールが自分で作ったファイル**である。クロール対象フォルダ内のファイルを削除する処理は無い。

確認コマンド（2.1 の `scan` を使う）:

```powershell
# 削除・書き込みの呼び出し箇所をすべて列挙し、対象のパスが work / tmpDir / diffTempRoot 由来であることを目で確かめる
scan 'Remove-Item','WriteAllText','WriteAllLines','StreamWriter','\.SaveAs','::Move','Move-Item'
```

### 3.2 取り込み・比較の対象のファイルは書き換えない

本ツールは、クロール対象フォルダのファイルや比べるファイルを**直接開かない**。作業フォルダへコピーし、そのコピーだけを開く（`shared/office/office_extract.ps1:105`・`258` の `copyFileShared`。インデクサと比較の抽出プロセスが同じ処理を使う）。コピー元は読み取り専用で開く（`shared/core/fs.ps1:109` の `copyFileShared`。`FileAccess::Read` で開き、ほかのアプリの読み書き・削除を妨げない）。
...
`tests/meta/safety.Tests.ps1` の「取り込み対象のファイルを書き換えないこと」（比較の対象も同じ処理で読む）が、原本のパスを書き込み・削除の API に渡さないこと、原本を読むのは `copyFileShared` の読み取りだけであること、`SaveAs` の保存先が作業フォルダだけであること、読み取り専用で開くことを確かめる。
...
対象フォルダに読み取りの権限だけを与えたアカウントで、インデックス作成（または比較）を最後まで実行する。書き込みを試みていれば失敗するため、完走することが「読み取りのみ」の裏付けになる。

---

## 4. 開示事項

次の 6 点は審査で論点になるため、本ツールの側から先に開示する。4.1・4.2・4.5 は利用者の操作に紐づく動作、4.3・4.4・4.6 はツールの性質上避けられない情報の扱いである。

### 4.1 Office プロセスの強制終了（［9 プロセス停止］タブ）

インデックス作成・比較の抽出が異常終了すると、画面の無い Excel・Word・PowerPoint のプロセスが残り、メモリを占有し続けることがある。これを止めるために、画面の［9 プロセス停止］タブから `Stop-Process -Force` を実行する（`shared/office/office_process.ps1:43`。`safety.Tests.ps1` がこの 1 か所だけであることを確かめる）。
...
起動用バッチ（`tebunko.bat`）は、画面を起動する前に、配布フォルダの `scripts` 配下の Mark-of-the-Web を解除し、そのあと画面を起動する。
...
- **範囲**: `%~dp0scripts`（`tebunko.bat` があるフォルダの `scripts`）配下のみ。実行するスクリプトはすべてここにある。インデックス（`work`）には触らない。システム全体やユーザーのドキュメントには影響しない。画面（`tebunko/gui.ps1`）も起動時に同じ `scripts` 配下に対して同じ解除を行う（ショートカットから直接起動したときのため）。
...
- **異常終了時**: 強制終了・停電などでコピーが残ることがある。この場合、**次回のインデックス作成開始時に、終了済みのプロセスの作業フォルダをまとめて削除する**（`tebunko_grep/indexer/index_migrate.ps1:29` の `removeStaleTmpDirs`。`tebunko_grep/indexer.ps1:97` で起動時に呼ぶ。`safety.Tests.ps1` の「異常終了で残った作業フォルダを次回起動時に回収する」が確かめる）。
- **残る期間**: 異常終了から次回のインデックス作成開始までの間は残る。気になる場合は `%TEMP%\tebunko_grep` を手で削除してよい（動作に影響しない）。

### 4.5 検索結果から開くと Excel が前面に出る

検索結果・比較の差分から元の Excel ファイルを開くときは、Excel を可視化して前面に出す（`shared/ui/open_file.ps1:58`・`109`）。利用者が開くよう操作したときの動作である。インデクサ・比較の抽出プロセス側の Office は常に不可視で動作する（2.2。`safety.Tests.ps1` が、`Visible = $true` は元のファイルを開く処理にしか無いことを確かめる）。
...
- **設定**: `setting.config` に `diff` で始まるキー（比べ方・表示の設定）が加わる。比べたファイル・フォルダのパスと本文は保存しない。
...
GitHub Release の配布 zip（`tebunko-<タグ>.zip`。展開すると `tebunko/` フォルダになる）は、`v` で始まるタグを push したときに `.github/workflows/release.yml` が作る。作る前に `test.yml` と同じ検査・テストを通す。zip の中身は次のとおり（`tools/new_release_package.ps1`）。`docs/`・`tests/`・`work/`・`setting.config` は入れない。

| ファイル | 内容 |
|---|---|
| `scripts/`・`tebunko.bat` | ツール本体 |
| `LICENSE`・`README.md`・`SECURITY.md`・`SECURITY.ja.md`・`sbom.cdx.json` | ライセンス・説明・脆弱性の連絡先（英語・日本語）・部品表 |
| `tebunko.cat`・`SHA256SUMS.txt` | 改ざんの確認用（`tools/new_release_files.ps1` が作る） |

**カタログ（証明書は不要）**: Windows PowerShell 5.1 標準の `New-FileCatalog` で、`scripts/` 配下すべてと `tebunko.bat` の SHA256 を 1 つのカタログにまとめてある。受け取った側は、zip を展開したフォルダで次を実行すれば、配布時点から 1 バイトも変わっていないことを確かめられる。
...
署名の bundle（`tebunko-<タグ>.zip.sigstore.json`）もリリースに載せている。GitHub に問い合わせずに確かめるときは `--bundle .\tebunko-v0.1.0.zip.sigstore.json` を付ける。zip 自体の SHA256 はリリースの説明に書いてある（5.6 の照会に使える）。
...
| 6 | `TypeNotFound` | `tebunko_grep/ui/types_grep.ps1` が、`shared/ui/types.ps1` の型（`NotifyBase`）を継承しているため。1 ファイルだけを解析すると継承元が見つからないという指摘で、実際の読み込み順では解決する（`tests/meta/structure.Tests.ps1` の「型の読み込み」で実際に読み込んで確認している）。どのルールを指定しても出る |
...
| 指標 | 値 |
|---|---|
| テスト件数 | 1,011 件（既定のタグ Unit・Io・Meta。失敗 0 件。うち安全性・構成の検査（タグ `Meta`）が 104 件） |
| コードカバレッジ | 96.5%（4,922/5,100 コマンド）。対象は画面の起動口・タブ・ダイアログを除くスクリプト（`tests/run.ps1` の `CodeCoverage` の条件）。Office の COM を使うインデクサは、COM の入口（`getApp`）を偽のオブジェクトに差し替えて検証している。検索結果の一覧・プレビューなど計測の対象に入る画面の部品は、画面のコントロールを偽のオブジェクトに差し替えて検証している。下限は `tests/coverage.baseline`（96.5%）で、下回ると CI が失敗する |
...
| 部品表（SBOM） | CycloneDX 1.6 形式で配布 zip に同梱。構成物は本ツール自身のファイルのみで、`purl`（パッケージ識別子）を持つ部品は 1 件も無い（`safety.Tests.ps1` が確かめる） |
...
| コミット前の検査 | 個人情報・文字コードの検査（`tools/check_commit.ps1`）と速いテストを pre-commit フックで動かす。CI は全ファイル・全履歴を検査する | `tools/hooks/`・`test.yml` |
...
| 依存の更新 | GitHub Actions と、設計書のサイトを作る Python パッケージ（`tools/mkdocs/requirements.txt`。ハッシュで固定し、配布物には入らない）は Dependabot が毎月 PR を出す。PowerShell のモジュール（Pester 3.4.0・PSScriptAnalyzer 1.25.0）は Dependabot の対象外のため、`test.yml` で版を固定する | `.github/dependabot.yml` |
...
| リリース | タグの push で、テストを通してから配布 zip を作り、カタログ・ハッシュ一覧・SBOM を同梱し、来歴に署名する（5.1） | `.github/workflows/release.yml` |
