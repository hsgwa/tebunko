...
| 場所 | 起動するもの | 用途 |
|---|---|---|
| `tebunko_grep/ui/index_tab.ps1:612`・`617`、`tebunko_grep/ui/open_source.ps1:311` | `explorer.exe` | 一覧・検索結果から元のファイルの場所を開く（利用者の操作時のみ） |
| `tebunko_grep/gui.ps1:379` | `powershell.exe` | 本ツール自身の画面 `tebunko_grep/gui.ps1` を開き直す（［8 設定］でワークスペースを変えたときだけ。`-NoProfile -STA -ExecutionPolicy RemoteSigned -WindowStyle Hidden`） |
