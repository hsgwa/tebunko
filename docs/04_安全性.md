...
|---|---|---|---|
| 構成物 | Windows PowerShell スクリプト（`scripts/**/*.ps1`）、画面定義（`*.xaml`）、起動用 `tebunko_grep.bat`、アイコン `tebunko_grep.ico` のみ。実行可能バイナリ（`.exe` / `.dll`）を同梱しない | 配布 zip の中身は `tools/new_release_package.ps1` が決める | [5.1](#51-配布物の完全性カタログハッシュ一覧来歴の署名) |
...
| 実行ポリシー | `Bypass` を使わず `RemoteSigned` で起動する。恒久的な変更（`Set-ExecutionPolicy`）をしない | `safety.Tests.ps1` | [4.2](#42-mark-of-the-web-の解除tebunko_grepbat) |
...
| 書き込み先 | `work/` 配下、`%TEMP%\tebunko_grep\<PID>` 配下、`setting.config`、利用者が指定した検索結果の出力先のみ | `safety.Tests.ps1` | [3.1](#31-書き込み削除する場所) |
...
| 規模（監査の目安） | `scripts/` 配下 50 ファイル・11,588 行（空行を除く）・378 関数。第三者依存が無いため、監査対象はこの範囲で閉じる | – | [5.7](#57-同梱の機械検査と品質の指標) |
...
検査対象は `scripts/` 配下のすべての `.ps1`（50 ファイル）である。下の確認コマンドは、説明のコメント行（「〜は使わない」などの記述）を除いて検索する。「該当 0 件」は、コマンドが 1 件も出力しないことを意味する。同じ検査を `tests/meta/safety.Tests.ps1` の「危険な処理を使っていないこと」が行い、CI（`test.yml` の `test`。main へのマージに必須）で PR ごとに実行する。
...
| 8 | 実行ポリシーの恒久変更・`Bypass` | 該当 0 件 | `scan 'Set-ExecutionPolicy','ExecutionPolicy\s+Bypass'` と `Select-String -Path .\tebunko_grep.bat -Pattern 'Bypass','Set-ExecutionPolicy'` |
...
| `Open` の `ReadOnly` | 真 | 読み取り専用で開く（`tebunko_grep/indexer/extract_office.ps1:122`（Excel）・`202`（Word）・`228`（PowerPoint）） |
...
| 場所 | 起動するもの | 用途 |
|---|---|---|
| `tebunko_grep/ui/index_tab.ps1:615`・`620`、`tebunko_grep/ui/open_source.ps1:311` | `explorer.exe` | 一覧・検索結果から元のファイルの場所を開く（利用者の操作時のみ） |
...
制限モード（[05_制限モード.md](05_制限モード.md)）は、次の 2 つを起動する。どちらも Windows に付いているもので、制限言語モードの制限を回避するものではない（`cmd.exe` と `tar.exe` は、スクリプトの言語モードに関係なく、アプリケーション制御の規則で許可された Windows の実行ファイルとして動く）。

| 場所 | 起動するもの | 用途 |
|---|---|---|
| `tebunko_grep/restricted/result_book.ps1` の `writeResultBook` | `cmd.exe` と `%windir%\System32\tar.exe` | 検索結果のブック（xlsx）の部品を ZIP にまとめる（作業フォルダに移り、`tar.exe --format zip -cf`） |
| `tebunko_grep/restricted/search_console.ps1` | `Invoke-Item` | 作った検索結果のブックと、利用者が一覧の番号で選んだ元のファイル・フォルダを、関連付けのアプリで開く |

いつもの画面も、インデックス作成ログ（`tebunko_grep/ui/indexing_tab.ps1`）・検索結果ファイル・元のファイル（`tebunko_grep/ui/open_source.ps1`）を `Invoke-Item` で開く。

元のファイルを開く操作（`tebunko_grep/ui/open_source.ps1:125`）では、`Start-Process` ではなく `ProcessStartInfo` を使う（`Start-Process` は `[` `]` を含むパスをワイルドカードとして解釈するため）。起動対象は、利用者が選んだ行のファイルと、それに関連付けられたアプリケーションである。
...
| 場所 | 内容 | 定義 |
|---|---|---|
| `work/` 配下（ツールを置いたフォルダの直下） | TSV インデックス（`work/index/`）、取り込み一覧・インデックス作成ログ・制御用ファイル、検索結果 | `shared/core/paths.ps1:5`、`tebunko_grep/core/paths_grep.ps1:9`・`14`・`22`〜`37` |
| `%TEMP%\tebunko_grep\検索結果` 配下 | 制限モードの検索結果のブック（検索ごとに作り、制限モードの起動時に消す。[05_制限モード.md](05_制限モード.md) の 4.4） | `tebunko_grep/restricted/result_book.ps1` の `${resultBookDir}` |
...
削除（`Remove-Item`）は 22 か所あるが、対象はすべて `work/` 配下・`%TEMP%\tebunko_grep\<PID>` 配下・`%TEMP%\tebunko_grep\検索結果` 配下（制限モードの結果のブック）、すなわち**本ツールが自分で作ったファイル**である。クロール対象フォルダ内のファイルを削除する処理は無い。
...
本ツールは、クロール対象フォルダのファイルを**直接開かない**。作業フォルダへコピーし、そのコピーだけを開く（`tebunko_grep/indexer/extract_office.ps1:103`・`256` の `copyFileShared`）。コピー元は読み取り専用で開く（`shared/core/fs.ps1:121`。`FileAccess::Read` で開き、ほかのアプリの読み書き・削除を妨げない）。
...
`tests/meta/safety.Tests.ps1` の「取り込み対象のファイルを書き換えないこと」が、原本のパスを書き込み・削除の API に渡さないこと、原本を読むのは `copyFileShared` の読み取りだけであること、`SaveAs` の保存先が作業フォルダだけであること、読み取り専用で開くことを確かめる。
...
次の 5 点は審査で論点になるため、本ツールの側から先に開示する。4.1・4.2・4.5 は利用者の操作に紐づく動作、4.3・4.4 はツールの性質上避けられない情報の扱いである。
...
`tebunko_grep.bat` は、画面を起動する前に、配布フォルダの `scripts` 配下の Mark-of-the-Web を解除し、そのあと画面を起動する。
...
rem Exit code 10: start the GUI below. Otherwise the restricted mode or a message (03_画面_5_共通仕様.md 10.1)
...
- **起動の確認**: Mark-of-the-Web を解除した後、画面を開く前に `start.ps1` で、いつもの画面を開けるか（言語モード・WPF）を確かめる。AppLocker・WDAC で PowerShell が制限言語モードになっている PC では、理由と管理者への頼み方を出して制限モードで起動する。グループポリシーの実行ポリシーで `start.ps1` 自体を動かせないときは、案内（`start_failed.txt`）と `Get-ExecutionPolicy -List` を表示して止まる。どちらも制限を回避しない（[03_画面_5_共通仕様.md 10.1](03_画面_5_共通仕様.md#101-配布と実行ポリシーmark-of-the-web)）。
- **理由**: zip で配布したものを展開すると全ファイルに Mark-of-the-Web が付き、実行ポリシー `RemoteSigned` では未署名スクリプトが実行できないため。
- **範囲**: `%~dp0scripts`（`tebunko_grep.bat` があるフォルダの `scripts`）配下のみ。実行するスクリプトはすべてここにある。インデックス（`work`）には触らない。システム全体やユーザーのドキュメントには影響しない。画面（`gui.ps1`）も起動時に同じ `scripts` 配下に対して同じ解除を行う（ショートカットから直接起動したときのため）。
...
検索結果から元の Excel ファイルを開くときは、Excel を可視化して前面に出す（`tebunko_grep/ui/open_source.ps1:162`・`213`）。利用者が開くよう操作したときの動作である。インデクサ側の Office は常に不可視で動作する（2.2。`safety.Tests.ps1` が、`Visible = $true` は `open_source.ps1` にしか無いことを確かめる）。
...
GitHub Release の配布 zip（`tebunko_grep-<タグ>.zip`）は、`v` で始まるタグを push したときに `.github/workflows/release.yml` が作る。作る前に `test.yml` と同じ検査・テストを通す。zip の中身は次のとおり（`tools/new_release_package.ps1`）。`docs/`・`tests/`・`work/`・`setting.config` は入れない。
...
**カタログ（証明書は不要）**: Windows PowerShell 5.1 標準の `New-FileCatalog` で、`scripts/` 配下すべてと `tebunko_grep.bat` の SHA256 を 1 つのカタログにまとめてある。受け取った側は、zip を展開したフォルダで次を実行すれば、配布時点から 1 バイトも変わっていないことを確かめられる。
...
| 項目 | 値 |
|---|---|
| 対象ファイル数 | 62（`scripts/` 配下すべてと `tebunko_grep.bat`） |
...
```powershell
gh attestation verify .\tebunko_grep-v0.1.0.zip -R hsgwa/tebunko
```

署名の bundle（`tebunko_grep-<タグ>.zip.sigstore.json`）もリリースに載せている。GitHub に問い合わせずに確かめるときは `--bundle .\tebunko_grep-v0.1.0.zip.sigstore.json` を付ける。zip 自体の SHA256 はリリースの説明に書いてある（5.6 の照会に使える）。
...
| 指標 | 結果 | 確認コマンド |
|---|---|---|
| 制限言語モード（Constrained Language Mode）で使えない書き方（静的解析） | **指摘 0 件**（`TypeNotFound` を除く）。ただしこのルールは `Add-Type`・`class`・COM・コア以外の .NET の型をほとんど指摘しないため、下の実動の確認を正とする | `Invoke-ScriptAnalyzer -Path .\scripts -Recurse -IncludeRule PSUseConstrainedLanguageMode` |
| 制限言語モードでの実動 | いつもの画面（`gui.ps1`）は 1 行目の `Add-Type` で止まる。起動の確認（`start.ps1`）と制限モードの部品は動く（本物の制限言語モード（WDAC）で CI の `clm.yml` が確かめる。模擬の制限言語モードで `tests/meta/clm.Tests.ps1` が確かめる） | [00_共通_3_テスト.md](00_共通_3_テスト.md) の `clm.yml`・`clm.Tests.ps1` |
...
制限言語モードを強制した環境では、COM オブジェクトの生成・`Add-Type`・PowerShell class が止まるため、いつもの画面とインデックス作成（Excel の COM を使う）は動かない。ツールのフォルダを管理者が許可すれば FullLanguage で動く。許可が無い PC では、起動の確認が理由を表示して制限モードで起動する（[03_画面_5_共通仕様.md 10.1](03_画面_5_共通仕様.md#101-配布と実行ポリシーmark-of-the-web)）。

制限モードでできること（[05_制限モード.md](05_制限モード.md)）は次のとおり。**制限言語モードを回避する手段（署名の偽装、許可された COM の悪用、`mshta` など）は使わない。**

| 機能 | 実現の仕方 | 使う外部プログラム |
|---|---|---|
| 検索 | TSV を `Get-Content -Raw` で読み、いつもの画面と同じ正規表現で照合する | なし |
| 検索結果の表示 | xlsx の部品（XML）を文字列で組み立て、ZIP にまとめて `Invoke-Item` で開く | `cmd.exe`・`tar.exe`（2.3） |
| インデックスの作成 | Office ファイル（新形式）を ZIP として展開し、XML から文字を取り出す。Excel のセルは表示形式を自前で当てる | `cmd.exe`・`tar.exe`（2.3） |

許可の仕方（AppLocker・WDAC のパス規則、実行ポリシー、共有フォルダとゾーン）は [05_制限モード_1_管理者向け.md](05_制限モード_1_管理者向け.md) にまとめてある。
...
```powershell
Get-FileHash .\tebunko_grep-v0.1.0.zip -Algorithm SHA256    # この値を VirusTotal の検索欄で照会する
```
...
| コードカバレッジ | 97.1%（5,994/6,175 コマンド）。対象は画面の起動口・タブ・ダイアログを除くスクリプト（`tests/run.ps1` の `CodeCoverage` の条件）。Office の COM を使うインデクサは、COM の入口（`getApp`）を偽のオブジェクトに差し替えて検証している。検索結果の一覧・プレビューなど計測の対象に入る画面の部品は、画面のコントロールを偽のオブジェクトに差し替えて検証している。下限は `tests/coverage.baseline`（96.6%）で、下回ると CI が失敗する |
| 規模 | `scripts/` 配下 50 ファイル・11,588 行（空行を除く）・378 関数 |
| 第三者依存 | 0 件 |

規模と依存の数を示す意味は、**監査にかかる手間を見積もれる**ことである。第三者依存が無いため、読む範囲はこの 11,588 行で閉じる。
