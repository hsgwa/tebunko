...
| 規模（監査の目安） | `scripts/` 配下 44 ファイル・10,164 行（空行を除く）・341 関数。第三者依存が無いため、監査対象はこの範囲で閉じる | – | [5.7](#57-同梱の機械検査と品質の指標) |
...
| `tebunko_grep/gui.ps1:309` | `powershell.exe` | 本ツール自身の画面 `tebunko_grep/gui.ps1` を開き直す（［8 設定］でインデックスの置き場所を変えたときだけ。`-NoProfile -STA -ExecutionPolicy RemoteSigned -WindowStyle Hidden`） |
...
| 規模 | `scripts/` 配下 44 ファイル・10,164 行（空行を除く）・341 関数 |
| 第三者依存 | 0 件 |

規模と依存の数を示す意味は、**監査にかかる手間を見積もれる**ことである。第三者依存が無いため、読む範囲はこの 10,164 行で閉じる。
