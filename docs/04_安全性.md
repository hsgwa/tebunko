...
GitHub Release の配布 zip（`tebunko_grep-<タグ>.zip`）は、`v` で始まるタグを push したときに `.github/workflows/release.yml` が作る。作る前に `test.yml` と同じ検査・テストを通す。展開したときに何を起動すればよいかが分かるよう、zip にはツール本体とライセンスだけを入れ、確認用のファイルは zip と並べてリリースに載せる（`tools/new_release_package.ps1`）。`docs/`・`tests/`・`work/`・`setting.config` はどちらにも入れない。README・SECURITY はリリースの説明からリンクする。
