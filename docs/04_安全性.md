...
| 書き込み先 | `work/` 配下（既定はツールのフォルダ。利用者が画面で選んだフォルダにも置ける）、`%TEMP%\tebunko_grep\<PID>` 配下、`setting.config`（ツールのフォルダに書き込めなければ `%LOCALAPPDATA%\tebunko\<鍵>`）、利用者が指定した検索結果の出力先のみ | `safety.Tests.ps1` | [3.1](#31-書き込み削除する場所) |
...
| 規模（監査の目安） | `scripts/` 配下 42 ファイル・10,144 行（空行を除く）・340 関数。第三者依存が無いため、監査対象はこの範囲で閉じる | – | [5.7](#57-同梱の機械検査と品質の指標) |
...
検査対象は `scripts/` 配下のすべての `.ps1`（42 ファイル）である。下の確認コマンドは、説明のコメント行（「〜は使わない」などの記述）を除いて検索する。「該当 0 件」は、コマンドが 1 件も出力しないことを意味する。同じ検査を `tests/meta/safety.Tests.ps1` の「危険な処理を使っていないこと」が行い、CI（`test.yml` の `test`。main へのマージに必須）で PR ごとに実行する。
...
| 場所 | 内容 | 定義 |
|---|---|---|
| `work/` 配下（既定はツールを置いたフォルダの直下。利用者が画面で選んだフォルダにも置ける） | TSV インデックス（`work/index/`）、取り込み一覧・インデックス作成ログ・制御用ファイル、検索結果 | `shared/core/data_dir.ps1:41`、`tebunko_grep/core/paths_grep.ps1:12`・`13`・`18`・`26`〜`41` |
...
| 規模 | `scripts/` 配下 42 ファイル・10,144 行（空行を除く）・340 関数 |
| 第三者依存 | 0 件 |

規模と依存の数を示す意味は、**監査にかかる手間を見積もれる**ことである。第三者依存が無いため、読む範囲はこの 10,144 行で閉じる。
