...
- **下限は `tests/coverage.baseline`（90.0）。** `.\tests\run.ps1 -Ci` がこれを下回ると失敗し、CI の必須チェック `test` が通らないためマージできない。
  ...
  - 実測が上がっても下限は上げない。重なったテストや数字のためだけのテストを消せる余地を残すため。下限を変えるのはメンテナだけ。
- **判断層は 95% 以上を保つ。** 画面に出す文言や可否の判定を足したら、同じ PR でテストを書く。
- **状態層・I/O もテストを書く。** 特にインデックスや設定ファイルの読み書きのように、壊れると利用者のデータが使えなくなる処理は、層にかかわらずテストを書く。
- **単体で確かめられることは単体テストで確かめる。** 起動口を通しで動かすテスト（`indexer.Tests.ps1` など）は、代表的な場面（基本の流れ・中止と再開・強制終了からの復旧・制限時間・元のファイルが無くなる）だけにする。
- **似たテストは表にまとめる。** 入力と期待値だけが違う `It` は、`-TestCases` の 1 つの `It` にする。
