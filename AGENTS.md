...
- 足したファイルは、必ず読み込み口（`shared/shared.ps1`・`tebunko_grep/lib.ps1`・`gui.ps1`・`indexer.ps1`・`start.ps1`）から読み込む。
- 制限モード（`start.ps1` と、そこから読み込むもの）は、制限言語モードでも読み込めて動く書き方だけで書く（`tests/meta/clm.Tests.ps1`）。
