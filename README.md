...
会社の PC で **AppLocker・WDAC（アプリケーション制御）** を強制している場合は、画面（ウィンドウ）が開けないことがあります。そのときは自動で**制限モード**（コンソール版）で起動し、理由を表示します。制限モードでもインデックスの作成と検索はできます（[制限モード](#制限モード)）。
...
会社の PC で AppLocker・WDAC（アプリケーション制御）を強制していると、許可されていない PowerShell は「制限言語モード」で動きます。この状態では画面（ウィンドウ）を開けません。tebunko は起動時にこれを調べ、開けないときは**制限モード**で起動して理由を表示します。
...
制限モードはコンソール（黒い画面）で動きます。**検索ワードを入れるだけで検索でき、結果は Excel のブックで開きます。**

| したいこと | 操作 |
|---|---|
| 検索する | 検索ワードを入れて Enter。結果のブックが Excel で開く |
| 検索の条件を変える | 空で Enter → `1`（正規表現・大文字と小文字・対象ファイル・図形・コメント） |
| 検索するインデックスを選ぶ | 空で Enter → `2` |
| インデックスを作る | 空で Enter → `3` |
| フォルダを追加・削除する | 空で Enter → `4` |
| 終わる | 空で Enter → `0` |

結果のブックは、ファイルごとにまとまっていて、一致した文字が赤く出ます。ヒットの行の ＋ で前後の行が開き、ファイル名のリンクで元のファイル（Excel は該当のシート・セル）が開きます。

**いつもの画面との違い**

| | いつもの画面 | 制限モード |
|---|---|---|
| 取り込めるファイル | Excel・Word・PowerPoint（旧形式・パスワード付きも） | 新形式（.docx / .docm / .pptx / .pptm / .xlsx / .xlsm）だけ |
| 検索 | できる | できる（同じインデックス・同じ結果） |
| 結果の表示 | 画面の一覧・プレビュー | Excel のブック（Excel が無ければコンソールに一覧） |
| インデックスの名前・場所の変更 | できる | できない（いつもの画面で行う） |

インデックスの形式はどちらのモードでも同じです。いつもの画面が使える PC で作ったインデックス（`work\index`）を共有フォルダに置けば、制限モードの PC からも検索できます。

管理者の方へ: 許可の仕方（AppLocker・WDAC のパス規則、実行ポリシー、共有フォルダとゾーン）は [管理者向けの案内](docs/05_制限モード_1_管理者向け.md) にあります。
...
- **制限モード：** 取り込めるのは新形式（.docx / .docm / .pptx / .pptm / .xlsx / .xlsm）だけです。Excel のセルの表示は、一部（和暦の書式・負の時刻）がいつもの画面と違います（[制限モード](#制限モード)）
