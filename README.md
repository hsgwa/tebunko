...
インデックス作成は、検索用のまとめファイル（`work\index`）とは別に、Windows Search に索引させる txt（`work\system_index`）を作ります。検索するときは、Windows Search で検索語を含みうるフォルダを先に絞り、その中だけを探します。結果は、すべてを探したときと同じです。検索ワードの下に `高速検索：使用可` / `使用不可` を表示します（正規表現・1 文字のワード・Windows Search が使えないときは使用不可で、すべてを探します）。

高速検索を早く効かせるには、`work\index` を Windows Search の対象から外してください（外さなくても結果は同じですが、Windows Search がまとめファイルの処理に時間を取られ、効くまで 1 日以上かかることがあります）。
