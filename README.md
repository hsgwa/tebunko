...
- **Office があれば、すぐに使える。** 必要なのは Windows と Microsoft Office だけです。インストールも、管理者権限も、追加のソフトも要りません。
- **すばやく探せる。** 100 万行（約 70MB）のインデックスでも、1〜2 秒で結果が出ます。
...
- **中身は PowerShell のスクリプトと画面の定義だけです。** 実行ファイルや、他者が作ったライブラリは含みません（[部品表](sbom.cdx.json)）。
...
sequenceDiagram
    actor U as 利用者
    participant T as tebunko
    participant F as Office ファイル<br>（共有フォルダなど）
    participant I as インデックス<br>（work/index/）

    Note over U,I: ① インデックスの作成（最初と、ファイルが変わったとき）
    U->>T: フォルダを登録して<br>［インデックス作成を開始］
    T->>F: コピーを読み取り専用で開く
    F-->>T: シート・ページ・スライドの文字
    T->>I: 文字だけを保存する

    Note over U,I: ② 検索（何度でも）
    U->>T: 検索ワードを入れる
    T->>I: インデックスの中を探す
    I-->>T: 一致した行
    T-->>U: ファイルごとの検索結果
...
- **インデックスの作成:** 登録したフォルダの Office ファイルを 1 つずつ開き、シート・ページ・スライドごとに文字だけを取り出して保存します。2 回目からは、前回から更新日時が変わったファイルと、新しく増えたファイルだけを読み込みます。
- **検索:** Office ファイルは開かず、インデックスの文字だけを探します。そのため、ファイルが何千あっても数秒で結果が出ます。
