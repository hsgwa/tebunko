...
インデックス・取り込み一覧・ログを置くフォルダ（**ワークスペース**）は、［8 設定］の［変更…］で、アクセス権を絞ったフォルダや容量のあるドライブの空のフォルダに変えられます。tebunko を書き込めない場所（`C:\Program Files` など）に置いたときは、設定とインデックスを利用者ごとの `%LOCALAPPDATA%\tebunko\` の下に置きます。
